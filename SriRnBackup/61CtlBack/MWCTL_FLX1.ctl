...
Begin VB.UserControl MWCTL_FLX 
   ...
   PropertyPages   =   "MWCTL_FLX1.ctx":0000
   ScaleHeight     =   3600
   ScaleLeft       =   100
   ScaleMode       =   0  'User
   ScaleWidth      =   5055
...
Dim ms_RecSourcetoUseLaterForReload As String
...
Event RowPostValid(ByVal RowNum As Integer, Cancel As Boolean, ErrMsg As String, ChangedColName As String)
...
Public Sub StartSpecialCopy()
  mf_SpecialCopy = True
End Sub

Public Sub EndSpecialCopy()

  MSFg.Redraw = True
  mf_SpecialCopy = False
End Sub
...
Public Property Get Value(ByVal pv_RowNum As Integer, ByVal pv_ColRef As Variant, Optional ByRef pr_Cancel As Boolean) As Variant
    ...
    If me_Mode = fgmServerCopy Then
      If mo_QueryObject Is Nothing Then err.Raise vbObjectError + 512, "", "No Command object present, Invalid access"
...
Public Function FormatText(ByVal pv_DataType As en_FldDataType, ByVal pv_FormatMask As String, ByVal pv_vNewValue As Variant)
    ...
    Case Is = fdtDateType
        FormatText = Format(pv_vNewValue, MWLib.GetDateFormatForEntry)
    Case Else
...
errh:
    ...
    me_Mode = fgmnorm
End Function
Public Sub RefreshData(Optional ByVal Reload As Boolean = False)
  LoadData (False)
End Sub
Private Sub LoadData(ByVal Reload As Boolean)
    ...
    If Reload = False Then
        ...
        ms_RecSourcetoUseLaterForReload = ms_RecSource
    Else
        ms_RecSource = ms_RecSourcetoUseLaterForReload
    End If
...
'    If Reload = False And ms_RecSource = ws_OrgRecSource Then Exit Sub
  ...
  '            j = j + 1
    ...
    ''***********************************************
    If MSFg.Rows > 1 Then Call RefChild(Reload)
...
Public Sub Store(ByVal pv_Fldname As String, ByVal pv_vNewValue As Variant)
    ...
    If wv_cmpval <> pv_vNewValue Then
        mf_DataChanged = True
        ms_Colname = ms_Colname + "," + pv_Fldname
    End If
...
Public Function SaveRec(Optional ByVal pv_RowNum As Variant) As Boolean
    ...
    Exit Function
err:
      err.Raise vbObjectError + 512, "", "Date format mismatch, Please quit the application and log back in"
      MsgBox "Date format mismatch, Please quit the application and log back in"
...
Private Function GetCol(ByVal pv_ColRef As String) As Integer
    ...
    Set wo_reqClsFlx = Nothing
End Function
Private Sub RefChild(Optional ByVal Reload As Boolean = False)
    Dim wv_rArr() As String, i As Integer
    wv_rArr = Split(ms_child, ",")
    If IsArray(wv_rArr) Then
        For i = 0 To UBound(wv_rArr)
            If IsNew(mi_PrevRow) Then Call Parent.Controls(wv_rArr(i)).Init
            Call Parent.Controls(wv_rArr(i)).RefreshData(Reload)
        Next i
    End If
End Sub
        ...
        If ws_DataFld <> "" Then
            If pv_Opt = ctm_Read Then
                ...
                Case Is = fdtFloat, fdtNumber, fdtTimeType
                  ...
                  End If
                Case Is = fdtDateType
                    MSFg.TextMatrix(pv_RowNum, i) = Format(IIf(IsNull(mo_Recset.FldValue(ws_DataFld)), "", mo_Recset.FldValue(ws_DataFld)), MWLib.GetDateFormatForEntry)
                Case Else
...
Private Sub Val_Col()
    ...
    If Not TXT_FLX.Locked And (me_Mode <> fgmCopy And me_Mode <> fgmCopyOver) Then
        Parent.adc.ValFailed = ""
...
'       err.Raise ctErrDtFormat, , "Date format switched inbetween sessions"
 ...
 'sri
    ...
    If MSFg.RowData(pv_RowNum) = 0 Or (mf_DataChanged) Then
        ...
        If MSFg.RowData(pv_RowNum) = 0 Or mf_DataChanged Then
          RaiseEvent RowPostValid(pv_RowNum, pr_Cancel, pr_ErrMsg, ms_Colname)
          If pr_Cancel = True Then err.Raise vbObjectError + 513, 0, pr_ErrMsg
        End If
        mo_Recset.Update
