...
'  GRP_REP.Add "(Process)Worker", Array("Td.TdByLoc+T d.TdByWrk", "max(Td.TdByLoc)+max(Td.TdByWrk)"), Array("'('+ RTrim(Td.TdByLoc)+ ') '+ Td.TdByWrk", "'('+ RTrim(max(Td.TdByLoc))+ ') '+ max(Td.TdByWrk)"), "TdByLoc,TdByWrk", "hTdByLoc,hTdByWrk", "Process", "", "", Array("(Select PDesc From Param Where PTyp= 'WORK' and PMCd= Td.TdByWrk)", "(Select PDesc From Param Where PTyp= 'WORK' and PMCd= max(Td.TdByWrk))")
  ...
  '*** Jay 3.2.0 [OdPoNo]
  GRP_REP.Add "Cust Sub PO", Array("OmCmCd+OmPONo+OdPoNo", "max(OmCmCd)+max(OmPONo)+max(OdPONo)"), Array("'('+ OmCmCd+ ') '+ LTrim(OmPONo)+(case when OdPONo<> '' then '/'+ LTrim(OdPONo) else '' end)", "'('+ max(OmCmCd)+ ') '+ LTrim(max(OmPONo))+(case when max(OdPONo)<> '' then '/'+ LTrim(max(OdPONo)) else '' end)"), "OmCmCd,OmPoNo,OdPoNo", "hOmCmCd,hOmPONo,hOdPoNo", "", "", "", Array("BOdTc+'/'+BOdYy+'/'+BOdChr+'/'+LTrim(Str(BOdNo))+'/'+LTrim(Str(BOdSr))+(case when OdPONo<> '' then ' - '+ LTrim(OdPONo) else '' end )", "max(BOdTc)+'/'+max(BOdYy)+'/'+max(BOdChr)+'/'+LTrim(Str(max(BOdNo)))+'/'+LTrim(Str(max(BOdSr)))+(case when Max(OdPONo)<> '' then ' - '+ LTrim(Max(OdPONo)) else '' end )")
  '*** Jay 3.2.0 [OdPoNo]
  ...
  '****** Sachin 2.14.0 - [23-11-2006] - [Multi-Company] - ******
    ...
    ' **** Manali 3.8.0
  If UCase(adc.MenuCd) = UCase("RepMetLsFg") Then
    GRP_REP.Add "Fg Date", Array("convert(char(10),FdDt,112)", "convert(char(10),max(FdDt),112)"), Array("convert(varchar, FdDt, 3)", "convert(varchar, max(FdDt), 3)"), "", "", "Quarter,Month,Week", "", "", ""
  ...
  Else
    GRP_REP.Add "Process", Array("Td.TdCoCd+Td.TdByLoc", "Td.TdCoCd+max(Td.TdByLoc)"), Array("'['+Td.TdCoCd+'] '+Td.TdByLoc", "'['+Td.TdCoCd+'] '+ max(Td.TdByLoc)"), "", "", "Company Code,Loc Prnt Code", "", "", Array("(Select LocDesc From Loc Where LocCoCd= Td.TdCoCd and LocTyp= 'P' and LocCd= Td.TdByLoc)", "(Select LocDesc From Loc with (index(PK_Loc)) Where LocCoCd= max(Td.TdCoCd) and LocTyp= 'P' and LocCd= max(Td.TdByLoc))")
    ...
    '*** These Groupings are below the Bag Level ###
    '*** (3.2.0)
    GRP_REP.Add "Date", Array("convert(char(10),Td.TdDt,112)", "convert(char(10),max(Td.TdDt),112)"), Array("convert(varchar, Td.TdDt, 3)", "convert(varchar, max(Td.TdDt), 3)"), "", "", "Quarter,Month,Week", "", "", ""
...
'                        Array("(case PValue when 'Y' then 'Flute Bag' when 'N' then 'Primary Bag' end)", "(case MAX(PValue) when 'Y' then 'Flute Bag' when 'N' then 'Primary Bag' end)"), "", "", "Bag Typ+Chr", "", "", Array("(case PValue when 'Y' then 'Flute Bag' when 'N' then 'Primary Bag' end)", "(case MAX(PValue) when 'Y' then 'Flute Bag' when 'N' then 'Primary Bag' end)")
'  GRP_REP.Add "Bag Character", Array("BChr", "max(BChr)"), Array("BChr", "max(BChr)"), "", "", "Bag Type,Bag Typ+Chr", "", "", Array("(Select PDesc from param where PTyp ='BCHR' and PMCd= BChr and PSCd= '')", "(Select PDesc from param where PTyp ='BCHR' and PMCd= max(BChr) and PSCd= '')")
...
'  space(1) as qTag, TdCoCd, TdBYy, TdBChr, TdBNo, TdRmWt as qLsWt,
'  0 as qMul, TdRmWt as qPureLsWt, 0.0 as qBGldWt, 0.0 as qPureBGldWt,
'  TDt, TdRmCd as qRmCd, TdFrRmDc as qRmDc, TdByWrk, LocPrntCd, TdByLoc,
'  TdFrRmLoc as qLsLoc, TdTc, TdYy, TdChr, TdNo, TdSr, DmCtg, DmPrdCtg,
'  BOdDmCd, OdKt, BOdTc, BOdYy, BOdChr, BOdNo, OmCmCd, OmPoNo, TdBQty, TdDustWt,
'  0.0 as qRecWt, 0.0 as qDustWt
'  From Txnd, Txn, RmMst, Bag, OrdDsg, OrdMst, DsgMst, Loc Where 1 = 2
  '*** Report Sql ***
...
' **** Zubin 211 **** '
  ...
  '*** Modified by Shilpa ***/
  If adc("UoYn1") = "" Then
    '*** This actually should have been: wBasicWt = "0"
    wBasicWt = "LrcDustWt"
    '*** For the following formula do not put the value as Zero directly
    '*** This actually should have been: GRP_REP.SetFormula Rep, "wDustWt", "0"
  ...
  '**********Geeta************emr206 ****24/04/04**If adc("UOYN1")="" then wSqlStrg6 = " 0 as LrcRecWt "
  '*********** wSqlStrg7 = " 0   as " + wBasicWt + " "
  If adc("UOYN1") <> "" Then
'*** (Bef 2.06)
'    wSqlStrg6 = " IsNull((Select LrcRecWt from LsRec where LrcCoCd = TdCoCd and " + _
'                "Substring(LTrim(Rtrim(Str(LrcYyMM))), 5, 2) = month(TDt) and " + _
'                "Substring(LTrim(Rtrim(Str(LrcYyMM))), 1, 4) = year(TDt) and " + _
'                "LrcLoc = TdByLoc and LrcRmCd = TdRmCd and LrcRmCtg = Rm.RmCtg and Str(" + wBasicWt + ", 16, 3) <> Str(0, 16, 3)), " + _
'                "IsNull((Select LrcRecWt from LsRec where LrcCoCd = TdCoCd and " + _
'                "Substring(LTrim(Rtrim(Str(LrcYyMM))), 5, 2) = month(TDt) and " + _
'                "Substring(LTrim(Rtrim(Str(LrcYyMM))), 1, 4) = year(TDt) and " + _
'                "LrcLoc = TdByLoc and LrcRmCd = '' and LrcRmCtg = Rm.RmCtg and Str(" + wBasicWt + ", 16, 3) <> Str(0, 16, 3)), 0)" + _
'                ") as LrcRecWt "
'
'    wSqlStrg7 = " IsNull((Select " + wBasicWt + " from LsRec where LrcCoCd = TdCoCd and " + _
'                "Substring(LTrim(Rtrim(Str(LrcYyMM))), 5, 2) = month(TDt) and " + _
'                "Substring(LTrim(Rtrim(Str(LrcYyMM))), 1, 4) = year(TDt) and " + _
'                "LrcLoc = TdByLoc and LrcRmCd = TdRmCd and LrcRmCtg = Rm.RmCtg " + _
'                "and Str(" + wBasicWt + ", 16, 3) <> Str(0, 16, 3)), " + _
'                "IsNull((Select " + wBasicWt + " from LsRec where LrcCoCd = TdCoCd and " + _
'                "Substring(LTrim(Rtrim(Str(LrcYyMM))), 5, 2) = month(TDt) and " + _
'                "Substring(LTrim(Rtrim(Str(LrcYyMM))), 1, 4) = year(TDt) and " + _
'                "LrcLoc = TdByLoc and LrcRmCd = '' and LrcRmCtg = Rm.RmCtg and Str(" + wBasicWt + ", 16, 3) <> Str(0, 16, 3)), 0)" + _
'                ") as " + wBasicWt + " "
...
'*** (Bef 2.06)
'  '/** selecting Loss where TdRmFrLoc = 'L' **/
'  wSqlStrg1 = "Select " + grpflds0 + ", " + _
'              "'D' as qTag, TdCoCd, TdBYy, TdBChr, TdBNo, TdRmWt as qLsWt, " + _
'              "(Case when TdFrRmDc= 'D' then 1 " + _
'              "when TdFrRmDc= 'C' then -1 Else 0 end) as qMul, " + _
...
'              "0.0 as qBGldWt, 0.0 as qPureBGldWt, TDt, TdRmCd as qRmCd, TdFrRmDc as qRmDc, " + _
'              "TdByWrk, TdByLoc, TdFrRmLoc as qLsLoc, TdTc, TdYy, TdChr, TdNo, TdSr, DmCtg, " + _
'              "DmPrdCtg, BOdDmCd, OdKt, BOdTc, BOdYy, BOdChr, BOdNo, OmCmCd, OmPoNo, " + _
'              "(Select TdBQty from Txnd where TdCoCd= Td.TdCoCd and TdTc= Td.TdTc and TdYy= Td.TdYy and " + _
'              "TdChr= Td.TdChr and TdNo= Td.TdNo and TdSr= Td.TdSr and TdSrNo= 0) as TdBQty, TdDustWt, " + wSqlStrg6 + ", " + wSqlStrg7 + _
'              " From Txnd Td Join Txn on TCoCd= TdCoCd and TTc=TdTc and TYy=TdYy " + _
'              "and TChr=TdChr and TNo=TdNo " + _
'              "Join RmMst Rm on RmCd=TdRmCd " + _
...
'              " where (select LocTyp from Loc where LocCoCd= TdCoCd and LocCd=TdFrRmLoc)='L' " + _
'              "and RmCtg = '" + adc("UoRmCtgFr") + "' " + _
'              "and TdSrNo<>0 " + wCnd
'*** (Bef 2.06)
...
'    ws_BagJoin = " Join Param with (Index (PK_Param)) On PTyp= 'BCHR' and PMCd= BChr and PSCd= '' and PValue = 'N' " + _
'              IIF(adc("UOBCHRFR") <> "", " and PMCd= '" + adc("UOBCHRFR") + "' ", " ")
'  ElseIf UCase(adc("UoBagTyp")) = "Y" Then
'    ws_BagJoin = " Join Param with (Index (PK_Param)) On PTyp= 'BCHR' and PMCd= BChr and PSCd= '' and PValue = 'Y' " + _
'              IIF(adc("UOBCHRFR") <> "", " and PMCd= '" + adc("UOBCHRFR") + "' ", " ")
'  Else
'    ws_BagJoin = " Join Param with (Index (PK_Param)) On PTyp= 'BCHR' and PMCd= BChr and PSCd= '' " + _
'              IIF(adc("UOBCHRFR") <> "", " and PMCd= '" + adc("UOBCHRFR") + "' ", " ")
...
'*** Jay 2.13(CT) ***
  ...
  '*** (Bef 19/02/08) commented by Jenny, instead the CmpStr property of the scope fields have been set appropriately
  ''****** Sachin 2.14.0 - [Multi-Company] - [27-11-2006] ******
  'ws_CoCnd = IIF(Trim(adc("UoCoCdFr")) <> "", " TCoCd >= '" + adc("UoCoCdFr") + "' ", "") + _
  '            IIF(Trim(adc("UoCoCdFr")) <> "" And Trim(adc("UoCoCdTo")) <> "", " And ", "") + _
  '            IIF(Trim(adc("UoCoCdTo")) <> "", " TCoCd <= '" + adc("UoCoCdTo") + "' ", "") + _
  '            IIF(Trim(adc("UoCoCdTo")) <> "" And Trim(adc("UoCoCdSel")) <> "", " And ", "") + _
  '            IIF(Trim(adc("UoCoCdSel")) <> "", " TCoCd In (" + adc("UoCoCdSel") + ") ", "")
  ''****** Sachin 2.14.0 - [Multi-Company] - [27-11-2006] ******
  '
  ...
  '
  'wAddlCnd = wAddlCnd + IIF(Trim(adc("UODTFR")) <> "01/01/80", " And Td.TdDt >= '" + Trim(adc("UODTFR")) + "'", "")
  'wAddlCnd = wAddlCnd + IIF(Trim(adc("UODTTO")) <> "01/01/80", " And Td.TdDt <= '" + Trim(adc("UODTTO")) + "'", "")
  ''****** Sachin 3.02.0 - Additional Conditions For New Fields
  '*** (Bef 19/02/08) commented by Jenny, instead the CmpStr property of the scope fields have been set appropriately
'
'      wAddlCnd = IIF(ADC("UoRmCtgFr") <> "", "  IsNull(Rm.RmCtg,'')>= '" + ADC("UoRmCtgFr") + "'", "")
'      wAddlCnd = wAddlCnd + IIF(ADC("UoRmCtgTo") <> "", " and IsNull(Rm.RmCtg,'')<='" + ADC("UoRmCtgTo") + "'", "")
'      wAddlCnd = wAddlCnd + IIF(ADC("UoRmCtgSel") <> "", " and IsNull(Rm.RmCtg,'') in (" + ADC("UoRmCtgSel") + ")", "")
'      If wAddlCnd <> "" Then _
'            wAddlCnd = " and (Rm.RmCtg Is Null  Or (" + wAddlCnd + " )) "
                     
        
      wAddlCnd = IIF(adc("UoRmCtgFr") <> "", "  Td.TdRmCtg>= '" + adc("UoRmCtgFr") + "'", "")
  ...
  If UCase(adc.MenuCd) = UCase("RepMetLsFg") Then
    ...
    ' ***** Manali 3.10.0 - 31/03/12 - Show Allowed Ls% in report Only When Process Grps Selected  (Not For FG Rep)
    wSqlStrg1 = "Select " + grpflds1 + ", " + _
     ...
     wSqlStrg1 = wSqlStrg1 + " ,'01/01/80' as TDt,'' as qRmCd, " + _
      ...
      wSqlStrg2 = " From Fgd " + _
                  ...
                  " Join DsgMst on DmIdNo=BDmIdNo and DmTcTyp= 'DM' and DmCd= BOdDmCd " + _
                  ...
                  " and FdYy+Str(FdKey,7)=(Select Max(M.FdYy+Str(M.FdKey,7)) From Fgd M Where M.FdCoCd=BCoCd and M.FdBYy=BYy and M.FdBChr=BChr and M.FdBNo=BNo and M.FdBIdNo=BIdNo " + IIF(gs_Partition = ctCurrPrtn, " and M.FdPrtKey=BPrtKey ", "") + ") " + _
                  " and TdSrNo > 0 " + ws_BagCnd + " Group By TdCoCd, TdBYy, TdBChr, TdBNo, TdRmCtg   " + _
                  "  /* Having Sum((Case When (TdToRmLocTyp= 'L' Or TdFrRmLocTyp= 'L') and not(TdFrRmLocTyp= 'L' and TdToRmLocTyp= 'L')  Then " + _
                  "                     (Case when TdToRmLocTyp= 'L' and TdToRmDc= 'D' then 1 " + _
                  "                     when TdToRmLocTyp= 'L' and TdToRmDc= 'C' then -1 " + _
                  "                     when TdFrRmLocTyp= 'L' and TdFrRmDc= 'D' then 1 " + _
                  "                     when TdFrRmLocTyp= 'L' and TdFrRmDc= 'C' then -1 Else 0 end) Else 0 End)* TdRmWt) >0 */  "
  ...
  Else
    ...
    wSqlStrg1 = "Select " + grpflds0 + ", " + _
                ...
                "TdByWrk, Lc.LocPrntCd as LocPrntCd, TdByLoc, (Case when L1.LocTyp= 'L' then TdFrRmLoc " + _
                "when L2.LocTyp= 'L' then TdToRmLoc Else '' end) as qLsLoc, " + _
                IIF(wb_ProcessGrpSel, " Lc.LocAllowLsPer ", " 0 ") + " as LocAllowLsPer, " + _
                "TdTc, TdYy, TdChr, " + _
    ...
    ' ***** Manali 3.9.0 - txn Join (commnted earlier) uncommented now - as query was slow in case date scope selected - Date compare strings changed to TDt from td.TdDt
      ...
      wSqlStrg3 = wSqlStrg3 + " max(Td.TdDt) as TDt, '' as qRmCd, '' as qRmDc, max(Td.TdByWrk) as TdByWrk, " + _
                  "max(Lc.LocPrntCd) as LocPrntCd, max(Td.TdByLoc) as TdByLoc, '' as qLsLoc, " + _
      ...
      wAddlCnd = wAddlCnd + IIF(adc("UoRmCtgTo") <> "", " and IsNull(Tr.TdRmCtg,'')<='" + adc("UoRmCtgTo") + "'", "")
      wAddlCnd = wAddlCnd + IIF(adc("UoRmCtgSel") <> "", " and IsNull(Tr.TdRmCtg,'') in (" + adc("UoRmCtgSel") + ")", "")
      If wAddlCnd <> "" Then _
      ...
      wSqlStrg4 = "/*From Txnd Td*/  From Txn  " + _
                  ...
                  '"Where " + IIF(gs_Partition = ctCurrPrtn, " TPrtKey='" + ctCurrPrtn + "' and ", "") + ws_CoCnd + IIF(Trim(ws_CoCnd) <> "", " And ", "") + wCnd + _
                  IIF(wAddlCnd <> "", IIF(wCnd <> "", " And ", " Where ") + wAddlCnd, "") + _
                  " and Td.TdPrdYN= 'Y' " + _
                  "And isnull(Rm.RmCtg, '" + adc("UoRmCtgFr") + "')= '" + adc("UoRmCtgFr") + "' " + _
                  "Group by Td.TdCoCd, Td.TdTc, Td.TdYy, Td.TdChr, Td.TdNo, Td.TdSr, Td.TdBYy, " + _
                  "Td.TdBChr, Td.TdBNo "
...
'
'    wSqlStrg1 = "Select " + grpflds1 + ", " + _
'                 "'D' as qTag, TdCoCd, TdBYy, TdBChr, TdBNo,  " + _
'                 "Sum((Case When (TdToRmLocTyp= 'L' Or TdFrRmLocTyp= 'L') and not(TdFrRmLocTyp= 'L' and TdToRmLocTyp= 'L')  Then " + _
'                 "                     (Case when TdToRmLocTyp= 'L' and TdToRmDc= 'D' then 1 " + _
'                 "                     when TdToRmLocTyp= 'L' and TdToRmDc= 'C' then -1 " + _
'                 "                     when TdFrRmLocTyp= 'L' and TdFrRmDc= 'D' then 1 " + _
...
'                 "                     when TdFrRmLocTyp= 'L' and TdFrRmDc= 'D' then 1 " + _
'                 "                     when TdFrRmLocTyp= 'L' and TdFrRmDc= 'C' then -1 Else 0 end) Else 0 End)* " + _
'                 " TdRmWt* RmPurityWt/ (case when Rm.RmCtg= 'G' then 0.995 when Rm.RmCtg= 'P' then 1 when Rm.RmCtg= 'S' then 1 when Rm.RmCtg= 'L' then 1 else 1 end)) as qPureLsWt, " + _
'                 " sum(TdRmWt *  (Case when TdFrRmLoc='W' and TdFrRmDc='D' then 1 " + _
'                 "                 when TdFrRmLoc='W' and TdFrRmDc='C' then -1 " + _
'                 "                 when TdToRmLoc='W' and TdToRmDc='D' then 1 " + _
'                 "                 when TdToRmLoc='W' and TdToRmDc='C' then -1 " + _
...
'                 "                 when TdToRmLoc='W' and TdToRmDc='D' then 1 " + _
'                 "                 when TdToRmLoc='W' and TdToRmDc='C' then -1 " + _
'                 "                 Else 0 end) * RmPurityWt/ " + _
'                  "(case when isnull(Rm.RmCtg, '')= 'G' then " + CStr(wf_BaseGldRmPurity) + " when isnull(Rm.RmCtg, '')= 'P' then " + CStr(wf_BasePltRmPurity) + " when isnull(Rm.RmCtg, '')= 'S' then " + CStr(wf_BaseSlvRmPurity) + " when isnull(Rm.RmCtg, '')= 'L' then " + CStr(wf_BasePdRmPurity) + " else 1 end)) 0.00 as qBPureGldWt "
'
'    wSqlStrg1 = wSqlStrg1 + ", '01/01/80' as TDt, '' as qRmCd, " + _
'                 "'' as qRmDc, " + _
'                 "'' as TdByWrk, '' as LocPrntCd, '' as TdByLoc, '' as qLsLoc, '' as TdTc, '' as TdYy, '' as TdChr, " + _
'                 "0 as TdNo, 0 as TdSr, max(DmCtg) as DmCtg, max(DmPrdCtg) as DmPrdCtg, max(BOdDmCd) as BOdDmCd, max(OdKt) as OdKt, max(BOdTc) as BOdTc, max(BOdYy) as BOdYy, max(BOdChr) as BOdChr, max(BOdNo) as BOdNo, max(OmCmCd) as OmCmCd, max(OmPoNo) as OmPoNo, " + _
'                 "max(BQty) as TdBQty, 0.0 as TdDustWt, " + wSqlStrg6 + ", " + wSqlStrg7
'
'  wCnd = IIF(gs_Partition = ctCurrPrtn, " FdPrtKey= '" + ctCurrPrtn + "' " + IIF(wCnd <> "", " and ", ""), "") + wCnd
'
...
'                "and OmYy= OdYy and OmChr= OdChr and OmNo= OdNo " + IIF(gs_Partition = ctCurrPrtn, " and OmPrtKey=OdPrtKey ", "") + _
'                "Join Bag  on BIdNo= FdBIdNo and BCoCd= FdCoCd and BYy= FdBYy and BChr= FdBChr and BNo= FdBNo " + IIF(gs_Partition = ctCurrPrtn, " and BPrtKey= FdPrtKey ", "") + _
'                "Join Txnd Td On TdCoCd=BCoCd and TdBYy=BYy and TdBChr=BChr and TdBNo=BNo " + IIF(gs_Partition = ctCurrPrtn, " and TdPrtKey= BPrtKey ", "") + " Join RmMst Rm on RmCd=TdRmCd " + IIF(gs_Partition = ctCurrPrtn, " and RmPrtKey=TdPrtKey ", "") + _
'                "Join DsgMst on DmIdNo=BDmIdNo and DmTcTyp= 'DM' and DmCd= BOdDmCd " + _
'                " and DmSz= '' " + IIF(gs_Partition = ctCurrPrtn, " and DmPrtKey=BPrtKey ", "") + _
'                "Left Outer Join InvFgd On IfCoCd=FdCoCd and IfFdBYy=FdBYy and IfFdBChr=FdBChr and IfFdBNo=FdBNo and IfFdIdNo=FdIdNo " + IIF(gs_Partition = ctCurrPrtn, " and IfPrtKey=FdPrtKey ", "") + _
'                "Left Outer Join InvHd On InCoCd=IfCoCd and InTc=IfTc and InYy=IfYy and InChr=IfChr and InNo=IfNo and InIdNo=IfInIdNo " + IIF(gs_Partition = ctCurrPrtn, " and InPrtKey=IfPrtKey ", "") + _
'                " Where " + wCnd + IIF(wCnd <> "", " and ", "") + _
'                " TdRmCtg= '" + ADC("UoRmCtgFr") + "' " + _
'                " and TdSrNo > 0 Group By TdCoCd, TdBYy, TdBChr, TdBNo "
'
'  End If
  
  '****** Sachin 3.01 MaxDop Option added below ******
  ' ***** Manali 3.9.0 - Option Force Order Commented
  If UCase(adc.MenuCd) = UCase("RepMetLsFg") Then
    wSqlStrg5 = " Order By TdCoCd, TdBYy, TdBChr, TdBNo  " + _
                "OPTION (/*FORCE ORDER,*/ MAXDOP 1) "
  Else
    ' ***** Manali 3.10.0 - 12/04/12 - Order By Changed to TdYyKey
    wSqlStrg5 = " Order By TdCoCd, TdYyKey, TdByy, TdBChr, TdBNo " + _
                "OPTION (MAXDOP 1) "
  
''    wSqlStrg5 = " Order By TdCoCd, TdTc, TdYy, TdChr, TdNo, TdSr, TdByy, TdBChr, TdBNo " + _
''                "OPTION (/*FORCE ORDER,*/ MAXDOP 1) "
  End If
    ...
    '*** (Jen 2.14)
      ...
      wSqlStrg = "set dateformat dmy " + wSqlStrg1 + wSqlStrg2 + " Union All " + wSqlStrg3 + wSqlStrg4 + wSqlStrg5
    
    '*** (Jen 2.14)
    Else
      wSqlStrg = "set dateformat dmy " + wSqlStrg1 + wSqlStrg2 + wSqlStrg5
  ...
  'h1PureGldWt
  'h2PureGldWt
  '/*g4PrnPureGldWt g3PrnPureGldWt g2PrnPureGldWt g1PrnPureGldWt gPrnPureGldWt*/
  ...
  'wSqlStrg = "set dateformat dmy " + _
             "Select Grp1, Grp2, Grp3, max(DGrp1), max(DGrp2), max(DGrp3), max(G1Desc), max(G2Desc), max(G3Desc), " + _
             "max(qTag), max(TdBYy), max(TdBChr), max(TdBNo), sum(qLsWt* qMul), " + _
             "max(qMul) as qMul, sum(qPureLsWt* qMul), sum(qBGldWt), sum(qPureBGldWt), " + _
             "max(TDt), max(qRmCd), max(qRmDc), max(TdByWrk), max(TdByLoc), max(qLsLoc), " + _
             "max(TdTc), max(TdYy), max(TdChr), max(TdNo), max(TdSr), max(DmCtg), " + _
             "max(DmPrdCtg), max(BOdDmCd), max(OdKt), max(BOdTc), max(BOdYy), max(BOdChr), " + _
             "max(BOdNo), max(OmCmCd), max(OmPoNo), " + _
             "sum(TdBQty), sum(TdDustWt), sum(qRecWt), sum(qDustWt) " + _
             "from (" + wSqlStrg1 + wSqlStrg2 + " Union All " + wSqlStrg3 + wSqlStrg4 + _
             ") RecSrc Group By Grp1, Grp2, Grp3 " + "OPTION (FORCE ORDER) "
  '###
    
  ''''Rep.PerformGroupingOnServer
