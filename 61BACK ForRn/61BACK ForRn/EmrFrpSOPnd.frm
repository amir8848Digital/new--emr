   ...
   Begin VB.Frame fra_nkeys 
      ...
      Begin TabDlg.SSTab TAB_REP 
         ...
         Begin VB.Frame fra_tabrep 
            ...
            Top             =   360
            Width           =   15105
            Begin VB.Frame FraBalValWIPFgQty 
               BorderStyle     =   0  'None
               Height          =   375
               Left            =   6420
               TabIndex        =   222
               ToolTipText     =   "Select Balance Value / WIP-Fg Qty"
               Top             =   8520
               Width           =   6195
               Begin VB.OptionButton OptBalValWIPFgQty 
                  Caption         =   "Bal Value"
                  BeginProperty Font 
                     Name            =   "Arial"
                     Size            =   9.75
               ...
               End
               Begin VB.OptionButton OptBalValWIPFgQty 
                  Caption         =   "WIP-Fg Qty (This will slow down the report)"
                  BeginProperty Font 
                     Name            =   "Arial"
                     Size            =   9.75
                     Charset         =   0
            ...
            Begin VB.Label LblValDivBy 
               ...
               Width           =   1785
            End
            Begin VB.Label LblBalValWIPFgQty 
               BackStyle       =   0  'Transparent
               Caption         =   "Show "
               BeginProperty Font 
                  Name            =   "Arial"
...
If UCase(ADC.MenuCd) <> UCase("RepSOPndRM") And UCase(ADC.MenuCd) <> UCase("RepJtSOPndRm") Then
  ...
  '6.1
  GRP_REP.Add "Order Date", "convert(varchar(10), BOm.OmDt, 112)", "convert(varchar(8), BOm.OmDt," + CStr(DtFmtSlahVal()) + ")", "OmDt", "hOmDt", "Quarter,Month,Week", "", "", ""
  ...
  If UCase(ADC.MenuCd) <> UCase("RepJtPndRm") And UCase(ADC.MenuCd) <> UCase("RepSOPndRM") And UCase(ADC.MenuCd) <> UCase("RepJtSOPndRm") Then
    ...
    '6.1 exp deldate, prd del date, customer PO, cust sub PO
    GRP_REP.Add "Exp Del Date", "convert(varchar(10), B.OdExpDelDt, 112)", "convert(varchar(8), B.OdExpDelDt," + CStr(DtFmtSlahVal()) + ")", "", "", "", "", "", ""
    GRP_REP.Add "Prd Del Date", "convert(varchar(10), B.OdDelDt, 112)", "convert(varchar(8), B.OdDelDt," + CStr(DtFmtSlahVal()) + ")", "OdDelDt", "", "", "", "", ""
    GRP_REP.Add "Customer PO", "BOm.OmPoNo", "", "OmPoNo", "hTBOENo", "Order Cust,Sales Exec", "", "", "convert(varchar(8), BOm.OmPoDt, " + CStr(DtFmtSlahVal()) + ")"
    '*** Jay 3.2.0 [OdPoNo]
    'vk.19 sub customer added
    GRP_REP.Add "Cust SUB PO", "BOm.OmPoNo+B.OdSubCust+B.OdPoNo", "BOm.OmPoNo+(case when B.OdPONo<> '' Or B.OdSubCust<>'' then '/'+ LTrim(B.OdSubCust)+' : '+ LTrim(B.OdPONo) else '' end )", "OmPoNo", "hTBOENo", "Order Cust,Sales Exec,Order", "", "", "convert(varchar(8), BOm.OmPoDt, " + CStr(DtFmtSlahVal()) + ")"
    '*** Jay 3.2.0 [OdPoNo]
    'VK.27 - Menu code "RepJtSoPndRm" added.
  ElseIf UCase(ADC.MenuCd) <> UCase("RepSOPndRM") And UCase(ADC.MenuCd) <> UCase("RepJtSOPndRm") Then
    ...
    '6.1 exp del date, customer PO
    GRP_REP.Add "Exp Del Date", "convert(varchar(10), B.OdExpDelDt, 112)", "convert(varchar(8), B.OdExpDelDt," + CStr(DtFmtSlahVal()) + ")", "", "", "", "", "", ""
    GRP_REP.Add "Customer PO", "BOm.OmPoNo", "", "OmPoNo", "HOmPoNo", "Order Cust,Sales Exec", "", "", "convert(varchar(8), BOm.OmPoDt, " + CStr(DtFmtSlahVal()) + ")"
  ...
  If UCase(ADC.MenuCd) = UCase("RepSOPndRM") Or UCase(ADC.MenuCd) = UCase("RepJtSOPndRm") Then
      ...
      '6.1
      GRP_REP.Add "Order Date", "convert(varchar(10), max(OmDt), 112)", "convert(varchar(8), max(OmDt), " + CStr(DtFmtSlahVal()) + ")", "max(OmDt)", "hOmDt", "Quarter,Month,Week", "", "", ""
...
'                   "On M.OdCoCd = B.OdCoCd And qBaseRefIdKey = B.OdIdNo " + IIF(gs_Partition = ctCurrPrtn, " and M.OdPrtKey=B.OdPrtKey ", "")
  ...
  'mw.133 when Show Wip-Fg qty option selected then getting Wip and FG qty
  Dim ws_WipFgQty As String
  If ADC("wBalValWipFgQty") = "N" Then
    ws_WipFgQty = ",(select isnull(sum(BQty),0) from Bag Join Param On PTyp= 'BCHR' and PMCd= BChr and PSCd= '' and PValue ='P' where BLoc<> 'PFG' and BCls ='N' and " + _
                  "BCoCd =OdCoCd and BOdTc =OdTc and BOdYy =OdYy and BOdChr =OdChr and BOdNo =OdNo and BOdSr =OdSr and BOdIdNo =OdIdNo ) as qWipQty " + _
                  ",(select isnull(sum(BQty),0) from Bag Join Param On PTyp= 'BCHR' and PMCd= BChr and PSCd= '' and PValue ='P' where BLoc= 'PFG' and BCls ='N' and " + _
                  "BCoCd =OdCoCd and BOdTc =OdTc and BOdYy =OdYy and BOdChr =OdChr and BOdNo =OdNo and BOdSr =OdSr and BOdIdNo =OdIdNo ) as qFgQty "
  End If
  ...
  If ADC("UoYN1") = "Y" Then
    ...
    'mw.133 ws_WipFgQty added
      wMtchStr = " IsNull(IdCoCd, '') as IdCoCd, IsNull(IdTc, '') as IdTc, IsNull(IdYy, '') as IdYy, " + _
                 ...
                 "OmNo= OdNo " + IIF(gs_Partition = ctCurrPrtn, " and OmPrtKey=OdPrtKey ", "") + " Join DsgMst on DmIdNo=OdDmIdNo and DmTcTyp= OdDmTcTyp and DmCd= OdDmCd and DmSz= '' " + IIF(gs_Partition = ctCurrPrtn, " and DmPrtKey=OdPrtKey ", "") + _
    ...
    Else
      wMtchStr = " IsNull(M.OdCoCd, '') as IdCoCd, IsNull(M.OdTc, '') as IdTc, IsNull(M.OdYy, '') as IdYy, " + _
                 ...
                 "BOm.OmNo= B.OdNo " + IIF(gs_Partition = ctCurrPrtn, " and BOm.OmPrtKey=B.OdPrtKey ", "") + " Join DsgMst on DmIdNo=OdDmIdNo and DmTcTyp= B.OdDmTcTyp and DmCd= B.OdDmCd and DmSz= '' " + IIF(gs_Partition = ctCurrPrtn, " and DmPrtKey=B.OdPrtKey ", "") + _
  ...
  Else
    ...
    wMtchStr = " '' as IdCoCd,'' as IdTc, '' as IdYy, '' as IdChr, 0 as IdNo, 0 as IdSr, '' as InDt, " + _
               ...
               "Join DsgMst on DmIdNo=OdDmIdNo and DmTcTyp= OdDmTcTyp and DmCd= OdDmCd and " + _
...
'  InExpNo, InCmCd, IdQty, idiFOB
  ...
  'sv.30 wValDivBy added to OdSalPrc
  If UCase(ADC.MenuCd) <> UCase("RepJtPndRm") And UCase(ADC.MenuCd) <> UCase("RepSOPndRM") And UCase(ADC.MenuCd) <> UCase("RepJtSOPndRm") Then
    '6.1
    wSqlStrg = "Set Dateformat " + DtFmtStr() + " Select " + grpflds + ", " + _
             ...
             "B.OdDmSz as OdDmSz, B.OdDmStkCoCd+'/'+B.OdDmStkYy+'/'+B.OdDmStkChr+'/'+ltrim(str(B.OdDmStkNo)) as OdDmStkNum, B.OdDmStkCoCd, B.OdDmStkYy, B.OdDmStkChr, B.OdDmStkNo, B.OdPicNm as OdPicNm, /*B.OdOrdQty*/ " + wOrdQty + " as OdOrdQty, " + IIF(wb_InBaseCur, " (Case when BOm.OmMulDiv= 'M' then (B.OdSalPrc/BOm.OmCnvFct) else (B.OdSalPrc*BOm.OmCnvFct) end) ", "B.OdSalPrc") + "/" + CStr(wValDivBy) + " as OdSalPrc, " + _
  ...
  '578.22 conversion factor added to OrSalRt & OrSalVal
    wSqlStrg = "Select " + grpflds + ", " + _
            ...
            "Convert(Decimal(14,3)," + IIF(wb_InBaseCur, " (Case when OmMulDiv= 'M' then OrSalRt/OmCnvFct else OrSalRt*OmCnvFct end) ", "OrSalRt") + "/" + CStr(wValDivBy) + ") OrSalRt, Convert(Decimal(14,3), " + wBalQty + " * " + IIF(wb_InBaseCur, " (Case when OmMulDiv= 'M' then OrSalVal/OmCnvFct else OrSalVal*OmCnvFct end) ", "OrSalVal") + "/" + CStr(wValDivBy) + ") OrSalVal " + _
            "From OrdRm " + _
...
'             IIF(wAddlCnd <> "", IIF(wCnd <> "", " And ", " Where ") + wAddlCnd, "") + wCrmCnd
  ...
  ADC("UoInVal") = "C"
  End If
      'sv.30 wValDivBy added to cost value & sale value
     If UCase(ADC("UoInVal")) = "C" And OptValWt(0).Value = True Then
        wOpt = "Cost Value"   '''"Actual"
        wIdFOBOpt = IIF(wb_InBaseCur, " (Case when OmMulDiv= 'M' then OdCstPrc/OmCnvFct else OdCstPrc*OmCnvFct end) ", "OdCstPrc") + "/" + CStr(wValDivBy)
       ' wIrRmWtOpt = "OrWt"
        wIrSetValOpt = IIF(wb_InBaseCur, " (Case when OmMulDiv= 'M' then OrSetCstVal/OmCnvFct else OrSetCstVal*OmCnvFct end) ", "OrSetCstVal") + "/" + CStr(wValDivBy)
        wIrRmValOpt = IIF(wb_InBaseCur, " (Case when OmMulDiv= 'M' then OrCstVal/OmCnvFct else OrCstVal*OmCnvFct end) ", "OrCstVal") + "/" + CStr(wValDivBy)
        wiLValOpt = IIF(wb_InBaseCur, " (Case when OmMulDiv= 'M' then OlCstVal/OmCnvFct else OlCstVal*OmCnvFct end) ", "OlCstVal") + "/" + CStr(wValDivBy)
     
     ElseIf UCase(ADC("UoInVal")) = "S" And OptValWt(0).Value = True Then
        wOpt = "Sale Value"      '''"Invoice"
        wIdFOBOpt = IIF(wb_InBaseCur, " (Case when OmMulDiv= 'M' then OdSalPrc/OmCnvFct else OdSalPrc*OmCnvFct end) ", "OdSalPrc") + "/" + CStr(wValDivBy)
       ' wIrRmWtOpt = "OrWt"
        wIrSetValOpt = IIF(wb_InBaseCur, " (Case when OmMulDiv= 'M' then OrSetSalVal/OmCnvFct else OrSetSalVal*OmCnvFct end) ", "OrSetSalVal") + "/" + CStr(wValDivBy)
        wIrRmValOpt = IIF(wb_InBaseCur, " (Case when OmMulDiv= 'M' then OrSalVal/OmCnvFct else OrSalVal*OmCnvFct end) ", "OrSalVal") + "/" + CStr(wValDivBy)
        wiLValOpt = IIF(wb_InBaseCur, " (Case when OmMulDiv= 'M' then OlSalVal/OmCnvFct else OlSalVal*OmCnvFct end) ", "OlSalVal") + "/" + CStr(wValDivBy)
  ...
  wStr2 = "From OrdDsg B join OrdMst BOm on OmIdNo=OdOmIdNo and OmCoCd= OdCoCd and OmTc= OdTc and OmYy= OdYy " + _
          ...
          " Join DsgMst on DmIdNo=OdDmIdNo and DmTcTyp= OdDmTcTyp and " + _
  ...
  GRP_REP.SetFormula mRep, "wDivByVal", ADC("UoValDivBy")
  
  'sv.30 when divied by amount>1 then follwing text will display in header
  If ADC("UoValDivBy") > 1 Then mRep.TxtDivByVal.SetText "(Values in " + Format(ADC("UoValDivBy"), "##,##,##0") + ")"
    
  
  'mw.133 if Show WIP&Fg Qty option is selected then Wip & Fg qty will be visible only for SO pending report
  If UCase(ADC.MenuCd) = UCase("RepSoPnd") And ADC("wBalValWIPFgQty") = "N" Then
    Rep.wBalVal1.Suppress = True:   Rep.gwBalVal.Suppress = True
    Rep.g1wBalVal.Suppress = True: Rep.g2wBalVal.Suppress = True
    Rep.g3wBalVal.Suppress = True: Rep.hBalVal.Suppress = True
    Rep.h1BalVal.Suppress = True
  Else
    Rep.qWipQty1.Suppress = True:   Rep.gqWipQty.Suppress = True
    Rep.g1qWipQty.Suppress = True: Rep.g2qWipQty.Suppress = True
    Rep.g3qWipQty.Suppress = True
    Rep.hWipQty.Suppress = True:    Rep.h1WipQty.Suppress = True
    Rep.qFgQty1.Suppress = True
    Rep.gqFgQty.Suppress = True:   Rep.g1qFgQty.Suppress = True
    Rep.g2qFgQty.Suppress = True:  Rep.g3qFgQty.Suppress = True
    Rep.hFgQty.Suppress = True:     Rep.h1FgQty.Suppress = True

  End If
  
'VK.27 - Menu code "RepJtSoPndRm" added.
If UCase(ADC.MenuCd) = UCase("RepSOPndRM") Or UCase(ADC.MenuCd) = UCase("RepJtSOPndRm") Then
    '62 HUID number will not needed for these reports
    Rep2.hHuidNo.Suppress = True:  Rep2.qHuidNo1.Suppress = True
    If ADC("UoYN5") = "Y" Then
...
End If
  ...
  If UCase(ADC.MenuCd) = UCase("RepJtPndRm") Then
    ws_BaseCurCd = moCn.GetFldVal("Select HBaseCurCd from Head where HCd= '" + ctSelfCmCd + "' and " + _
                    "HCoCd= '" + ADC("UoCoCdFr") + "'")
    If wb_InBaseCur Then
      Rep1.TxtValIn.SetText "Value In " + ws_BaseCurCd
    Else
      Rep1.TxtValIn.SetText "Value In " + ADC("UoCurCdFr")
    End If
  End If
...
'    Rep1.PicSec.Suppress = True
  ...
  End If
   ...
   GRP_REP.SetFormula Rep1, "wRmCdFontSz", GetRmcdFontSz("{rdo.OrRmCd}", 7)
...
'    ' ***** Manali Trading Module
    ...
    ' ###########################################  Manoj  ###########################################
      ...
      'mw.133 scope of Show Wip&FG qty will be visible for so pending only
      LblBalValWIPFgQty.Visible = True: FraBalValWIPFgQty.Visible = True
    ...
    ElseIf UCase(ADC.MenuCd) = UCase("RepJtPnd") Or UCase(ADC.MenuCd) = UCase("RepJtOpnOrd") Or _
      ...
      End If
      'mw.133 scope of Show Wip&FG qty will be visible for so pending only
      LblBalValWIPFgQty.Visible = False: FraBalValWIPFgQty.Visible = False
      ...
      ElseIf UCase(ADC.MenuCd) = UCase("RepSOPndRM") Or UCase(ADC.MenuCd) = UCase("RepJtSOPndRm") Then
        ADC("UoOmTc").Enabled = False: ADC("UoOmTc").Visible = False
        gs_DmTcTyp = "DM"
          If UCase(ADC.MenuCd) = UCase("RepJtSOPndRm") Then
            gs_OmTcTyp = ctJtTcTyp
          Else
            gs_OmTcTyp = "SO"
          End If
...
Private Sub OptBalValWIPFgQty_Click(Index As Integer)
  'mw.133 values for Show Wip/Fg Qty option
  Select Case Index
  Case Is = 0
    ADC("wBalValWipFgQty") = "Y"
  Case Is = 1
    ADC("wBalValWipFgQty") = "N"
  End Select
End Sub
'mw.133 Tooltip for  for Show Wip/Fg Qty option
Private Sub OptBalValWIPFgQty_GotFocus(Index As Integer)
  DispMsg FraBalValWIPFgQty.ToolTipText, etInfo
End Sub
...
'Thirumozhi
  ...
  End If
  'mw.133 - default value for show Bal Value
  OptBalValWIPFgQty(0).Value = True
  ADC("wBalValWipFgQty") = "Y"
