...
'*** Report Sql ***
  ...
  wSqlStrg = "Select Max(IdVaCtg+' '+ParamForVACtg.PDesc+' '+'HSN '+ParamForVACtg.PDesc225) as qGrp2, InTc, InYy, InChr, InNO, Max(" + ws_IdSr + ") as IdSr, Max(InPrnDt) as InDt,  " + _
   ...
   "max(InAmt9) as InAmt9, max(InAmt10) as InAmt10, max(InAmt11) as InAmt11,  max(InAmt12) as InAmt12, max(InGstCd) as InGstCd,  max(InBaseAmt) as InBaseAmt," + _
    ...
    "Max(OdPicNm) as OdPicNm,Max(IdSfx) as IdSfx,Max((Case When OdPoNo<>'' Then OdPoNo Else OmPoNo End)) as OdPoNo," + _
    " Max(IDTrayNo) as IDTrayNo, Max(ParamForDmctg.PDesc) as DmCtgDesc, Max(DmUom) as DmUom, " + IIF(wGrpDescFormula = "", "''", wGrpDescFormula) + " as wGrpDescFormula,Max(OdMrp) as OdMrp,Max(OdMrpDisc) as OdMrpDisc,"
  ...
  wSqlStrg = "Select Max(OdVaCtg+' '+ParamForVACtg.PDesc+' '+'HSN '+ParamForVACtg.PDesc225) as qGrp2, OmTc as InTc, OmYy as InYY, OmChr as InChr, OmNO as InNO, max(" + ws_IdSr + ") as IdSr, Max(IsNull(InPrnDt, OmDt)) as InDt,  " + _
   ...
   "max(InAmt9) as InAmt9, max(InAmt10) as InAmt10, max(InAmt11) as InAmt11,  max(InAmt12) as InAmt12,  max(InGstCd) as InGstCd,  max(InBaseAmt) as InBaseAmt, " + _
    ...
    "Max(OdPicNm) as OdPicNm,Max(OdSfx) as IdSfx,Max((Case When OdPoNo<>'' Then OdPoNo Else OmPoNo End)) as OdPoNo, " + _
    "0 as IDTrayNo, Max(ParamForDmctg.PDesc) as DmCtgDesc, Max(DmUom) as DmUom, " + IIF(wGrpDescFormula = "", "''", wGrpDescFormula) + " as wGrpDescFormula,Max(OdMrp) as OdMrp,Max(OdMrpDisc) as OdMrpDisc, "
