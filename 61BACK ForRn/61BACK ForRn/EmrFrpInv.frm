      ...
      Begin TabDlg.SSTab TAB_REP 
         ...
         Begin VB.Frame fra_tabrep 
            ...
            Begin VB.CheckBox ChkBoxArr 
               ...
               TabIndex        =   14
               Tag             =   "ShowCustPoDsg"
               ToolTipText     =   "Check To Show Customer PO Number"
               Top             =   2475
               Width           =   3240
            End
            Begin VB.CheckBox ChkBoxArr 
...
'*** (Jen 2.12)
  ...
  Dim wSubRepGrpFormula As String, wDetDesc3Formula As String
...
'    wDetDesc2Formula = ""
  ...
  ElseIf UCase(adc("UoInDet")) = "INVSR" Then
    ...
    '*** Jay 3.3(FG)  '6.1b-37
    wDetDesc2Formula = MakeCustDsgCdPoStr()
...
'    Else
  ...
  wHdStr1 = wHdStr1 + "max(Ih1.InHItcNo) as InHItcNo, max(Ih1.InHEtcZone) as InHEtcZone, max(Ih1.InHEtcSection) as InHEtcSection, max(Ih1.InHConsignee) as InHConsignee, " + _
            ...
            "max(Ih1.InCircularNo) as InCircularNo, sum(Ih1.InTCS) as InTCS,sum(Ih1.InTCSRs) as InTCSRs, "
  '*** Jay 3.2.0 [OdPoNo] '*** Jay 3.2.0 [OdPicNm]
  ...
  wHdStr1 = wHdStr1 + "'' as OmCmCd, '' as IdKt, '' as DmCtg, '' as DmSalCtg, '' as IdDmCd, '' as IdSfx, '' as IdDmSz, '' as DmUOM, " + _
            ...
            "'' as RmCtg, '' as RmKt, 0 as qRmRt, 0 as IrRmQty, 0 as qWtInCts, 0 as qWtInGms, 0 as qVal, 0 as IdTrayNo, '" + UCase(MWLib.DtSetting) + "' as qDtsetting " + _
            "from InvHd Ih " + _
  ...
  '*** Jay 3.3(FG)
    ...
    wStr1 = wStr1 + "max(Ih1.InCls) as InCls, max(Ih1.InHBkName) as InHBkName, max(Ih1.InHBkAdd1) as InHBkAdd1, max(Ih1.InHBkAdd2) as InHBkAdd2, max(Ih1.InHBkAdd3) as InHBkAdd3, " + _
            ...
            "max(Ih1.InHHideCommYN) as InHHideCommYN, max(Ih1.InHSelfDscYN) as InHSelfDscYN, max(Ih1.InHSelfDeductYN) as InHSelfDeductYN, max(Ih1.InCircularNo) as InCircularNo, 0 as InTCS,0 as InTCSRs,"
    '*** Jay 3.1.1 [Discount]
    ...
    wStr3 = wStr3 + "sum(IrSetZVal) as qZSetVal, " + wZLabValStr + " as qZLabVal, max(IdZFOB) as IdZFOB, " + qMainMet + ", " + qMainRmCtgDesc + ", " + qDmCtgDesc + ", " + qDmSalCtgDesc + ", " + qDmColDesc + ", " + qDmKtDesc + ", " + qVaCtgDesc + ", max(Ih1.InMMTC) as InMMTC," + _
            ...
            " '' as RmCtg, '' as RmKt, 0 as qRmRt, 0 as IrRmQty, 0 as qWtInCts, 0 as qWtInGms, 0 as qVal, Max(IdTrayNo) as IdTrayNo,'" + UCase(MWLib.DtSetting) + "' as qDtsetting " + _
...
'    If UCase(adc("UoYN10")) = "Y" And ChkBoxArr(10).Visible = True Then mRep.TxtBinNo.SetText "CE Regn. No."
  ...
  '*** Jay 3.3(FG)
    ...
    GRP_REP.SetFormula mRep, "wDetDesc3", wDetDesc3Formula    '6.1b-37
    '***Geeta***Emr208
...
'    '******************************** urmi *********************************************/
    '6.1B-37 adc("UoYN15") added
    If UCase(adc("UoYN1")) = "Y" Or UCase(adc("UoYN15")) = "Y" Then
...
'    End If
  ...
  End If
  ' **** Manali 3.03 - 26/06/08 *****
  'sv.44 showing Tcs details
  Call ShowTcsDet
  '578.19 calling function to change Rmcode font size
  SetSubFormula Rep.Subreport2, "wRmCdFontSz", GetRmcdFontSz("{Command.RmCd}", 7)
...
''      ChkBoxArr(4).Enabled = False: ChkBoxArr(4).Visible = False  ' **** Manali 3.5.0 - 18/11/08 - Check Boxes Added
   ...
   Case Is = UCase("RepInvLoose")
    ...
    '*** Jay 3.3(FG)
      ...
      ChkBoxArr(15).Enabled = False: ChkBoxArr(15).Visible = False    '6.1b-37
      '******************************** urmi *********************************************/
...
'                                     "HCd='" + ctSelfCmCd + "'"))
  ...
  If ChkBoxArr(15).Visible = True And adc("UOYN15") = "" Then adc("UOYN15") = "N"   '6.1b-37
  ...
  If adc("UoYN15") = "Y" Then ChkBoxArr(15).Value = Checked Else ChkBoxArr(15).Value = Unchecked    '6.1b-37
  ' **** Manali 3.5.0 - 18/11/08 - Check Boxes Added
End Sub
...
Private Sub GetStnClsSumm(ps_AndCnd As String)
  ...
  wStnClsSummStr = "Select IrTc, IrYy, IrChr, IrNo, PDesc as qRmStnCls, " + _
               ...
               "where IrRmCtg In ('D','C') " + IIF(gs_Partition = ctCurrPrtn, " and IrPrtKey='" + ctCurrPrtn + "' ", "") + _
                ps_AndCnd + " Group By IrTc, IrYy, IrChr, IrNo, IrRmCtg , RmStnCls,PDesc order by  IrTc, IrYy, IrChr, IrNo, IrRmCtg desc, qRmStnCls "
  
  Call moCn.RepRes(mRep.Subreport4.OpenSubreport, wStnClsSummStr + ctMaxDopOpt)

End Sub
Private Sub ShowTcsDet()
  'sv.44 update TCS description
  Dim wTCSPer As Double
  wTCSPer = moCn.GetFldVal("select InTCSPer from InvHd where InCoCd ='" + adc("UoCoCdFr") + _
                    "' and InTC='" + adc("UoInTcFr") + "' and InYy ='" + adc("UoInYyFr") + _
                    "' and InChr ='" + adc("UoInChrFr") + "' and InNo =" + CStr(adc("UoInNoFr")))
  
  Rep.TCSDesc.SetText "@" + CStr(wTCSPer) + "%TCS"
  If wTCSPer = 0 Then Rep.TCSDesc.Suppress = True
...
'6.1b-37
Private Function MakeCustDsgCdPoStr() As String
  If adc("UOYN1") = "Y" Then
    MakeCustDsgCdPoStr = IIF(adc("UOYN1") = "Y", "{rdo.OmPoNo}+" + "(If {rdo.OdPoNo}= '' then '' else ' - ') +Trim({rdo.OdPoNo})", "") + _
                        IIF(adc("UOYN15") = "Y", IIF(adc("UOYN1") = "Y", "+", "") + "(If {rdo.CdCustDmCd}= '' then '' else ' /   ') +Trim({rdo.CdCustDmCd})", "")
  Else
    MakeCustDsgCdPoStr = IIF(adc("UOYN15") = "Y", "Trim({rdo.CdCustDmCd})", "")
  End If
End Function
