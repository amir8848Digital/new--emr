...
'      From InvHd, TxndZ Where 1 = 2
  ...
  wHdStr2 = " max(Ih1.InCls) as InCls, max(Ih1.InHBkName) as InHBkName, max(Ih1.InHBkAdd1) as InHBkAdd1, " + _
            ...
            "max(Ih1.InHHideCommYN) as InHHideCommYN, max(Ih1.InHSelfDscYN) as InHSelfDscYN, max(Ih1.InHSelfDeductYN) as InHSelfDeductYN, max(Ih1.InHSCode) as InHSCode  "
  ...
  Else
    ...
    If Not (UCase(adc.MenuCd) = UCase("RepGSPDetLet") Or UCase(adc.MenuCd) = UCase("RepGSPDetLetMulti")) Then
      wStr3 = ", sum(IsNull(M.TzPurAmt, 0) * (case when IsNull(RmCtg, '') in ('D') then 1 else 0 end)) as qDiaVal, " + _
              ...
              "sum(IsNull(M.TzPurAmt, 0) * (case when IsNull(RmCtg, '') in ('L') then 1 else 0 end)) as qPdVal, " + _
    ...
    Else
      wStr3 = ", sum(IsNull(M.TzPurAmt, 0) * (case when IsNull(TImpCtg, '')='IMP' and IsNull(RmCtg, '') in ('D') then 1 else 0 end)) as qDiaVal, " + _
              ...
              "sum(IsNull(M.TzPurAmt, 0) * (case when IsNull(TImpCtg, '')='IMP' and IsNull(RmCtg, '') in ('L') then 1 else 0 end)) as qPdVal, " + _
              ...
              "sum(IsNull(M.TzPurAmt, 0) * (case when IsNull(TImpCtg, '')='IND' and IsNull(RmCtg, '') in ('L') then 1 else 0 end)) as qPdIndVal, " + _
    ...
    wFinalStr2 = " max(InCls) as InCls, max(InHBkName) as InHBkName, max(InHBkAdd1) as InHBkAdd1, " + _
                 ...
                 "max(InHHideCommYN) as InHHideCommYN, max(InHSelfDscYN) as InHSelfDscYN, max(InHSelfDeductYN) as InHSelfDeductYN, max(InHSCode) as InHSCode  "
