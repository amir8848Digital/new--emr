...
Private Sub SetGroupSort()
  ...
  GRP_REP.Add "Design Code", "max(OdDmCd)", "", "", "", "Design Ctg", "", "", "(Select Substring(DmDesc,1,120) From DsgMst Where " + IIF(gs_Partition = ctCurrPrtn, " DmPrtKey='" + ctCurrPrtn + "' and ", "") + " DmTcTyp= 'DM' and DmCd= max(OdDmCd) and DmSz= '') "
  ...
  '*** Jay 3.2.0 [OdPoNo]
  GRP_REP.Add "Cust Sub PO", "max(OmCmCd)+max(OmPONo)+max(OdPONo)", "'('+ max(OmCmCd)+ ') '+ LTrim(max(OmPONo))+(case when Max(OdPONo)<> '' then '/'+ LTrim(Max(OdPONo)) else '' end )", "", "", "Customer Code", "", "", "OdTc+'/'+OdYy+'/'+OdChr+'/'+LTrim(Str(OdNo))+'/'+LTrim(Str(OdSr))+(case when max(OdPONo)<> '' then '/'+ LTrim(max(OdPONo)) else '' end )"
  '*** Jay 3.2.0 [OdPoNo]
  ...
  GRP_REP.Add "(DsgCd)Colour", "max(OdDmCd)+max(OdDmCol)", "'('+max(OdDmCd)+') '+max(OdDmCol)", "", "", "Design Ctg", "", "", "(Select Substring(DmDesc,1,120) From DsgMst Where " + IIF(gs_Partition = ctCurrPrtn, " DmPrtKey='" + ctCurrPrtn + "' and ", "") + " DmTcTyp= 'DM' and DmCd= max(OdDmCd) and DmSz= '') "
  
  'Sachin 3.11.0
  GRP_REP.Add "Design Colour", "max(OdDmCol)", "max(OdDmCol)", "", "", "", "", "", "(Select PDesc from Param where PTyp= 'DMCOL' and PMCd= max(OdDmCol))"
  GRP_REP.Add "(DsgCd)Size", "max(OdDmCd)+max(OdDmSz)", "'('+max(OdDmCd)+') '+max(OdDmSz)", "", "", "Design Ctg", "", "", "(Select Substring(DmDesc,1,120) From DsgMst Where " + IIF(gs_Partition = ctCurrPrtn, " DmPrtKey='" + ctCurrPrtn + "' and ", "") + " DmTcTyp= 'DM' and DmCd= max(OdDmCd) and DmSz= '') "
  
  GRP_REP.Add "Sales Exec", "max(CmSalPer)", "", "", "", "", "", "", "(Select PDesc from Param Where PTyp = 'SP' and PMCd= max(CmSalPer)) "
  '/** urmi 2.06 PrdSeq*//
...
'    wStr = " where OdPrdQty> OdFgQty and IsNull(BLoc,'')<> '" + ctFgLoc + "' "
  ...
  ws_CustDmCdStr = "IsNull((Select cast(CdCustDmCd as Varchar(17)) from CustDsg " + _
...
' ****** Bhavna emr 5.1.0 Wax set and hand set diamonds available for ..If diamond status Y
  ...
  wSqlStrg = "Set DateFormat DMY " + _
            ...
            "Where RC1.PTyp='REPCOL'  and DateDiff(dd, '" + CStr(moCn.SrvrDate) + "', max(" + wDelDt + ")) >= WC1.PNum and DateDiff(dd, '" + CStr(moCn.SrvrDate) + "', max(" + wDelDt + "))<= WC1.PNUm1), '') as qOmColCrCd, " + _
             ...
             wStr + wCnd + IIF(gs_Partition = ctCurrPrtn, " and OdPrtKey='" + ctCurrPrtn + "' ", "") + IIF(wAddlCnd <> "", IIF(wCnd <> "", " And ", "") + wAddlCnd, "") + wCrmCnd + " and exists (select 'x' from param where PTyp= 'TC' and PMCd= OdTc and PValue= 'SO') " + _
...
Private Function FldLst(ByVal mSeq As String, ByVal mCtr As Integer)
  ...
  Set wRsLocDesc = moCn.OpenRes("Select LocCd, LocDesc from Loc where LocCoCd ='" + IIF(adc("UOYN2") = "Y", ctSelfCoCd, IIF(wi_ScopeCos = 1, ws_ScoCnd, ctSelfCoCd)) + "'")
