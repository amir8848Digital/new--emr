...
Private Sub adc_setreprecsource()
  ...
  Dim wsql3 As String, wSqlStrg As String
  Set Rep = Nothing
  ...
  If UCase(ws_Opt) = UCase("RepRmRt") Then
    ...
    wsql1 = "Select 'R' as qTag, " + _
            ...
            "From RmRt " + wCnd + IIF(gs_Partition = ctCurrPrtn, IIF(wCnd <> "", " and ", " Where ") + " RrPrtKey='" + ctCurrPrtn + "' ", "")
  ...
  ElseIf UCase(ws_Opt) = UCase("RepRmRtHist") Then
    ...
    wsql1 = "Select 'R' as qTag, " + _
            ...
            "From RmRtHist " + wCnd + IIF(gs_Partition = ctCurrPrtn, IIF(wCnd <> "", " and ", " Where ") + " RhPrtKey='" + ctCurrPrtn + "' ", "")
...
' to avoid the blank page coming for p Customers when there is no rec in rmRt for them
  If UCase(ws_Opt) = UCase("RepRmRt") Then
    wsql3 = " Exists(Select 'x' from RmRt where " + IIF(gs_Partition = ctCurrPrtn, " RrPrtKey='" + ctCurrPrtn + "' and ", "") + " RrTcTyp= TsTyp and RrCtg= TsSTyp and " + _
            " RrCmCtg= '" + adc("UoCmCtgFr") + "') " + wCnd + _
            " group by TsTbl, TsTyp, TsSTyp "
  ElseIf UCase(ws_Opt) = UCase("RepRmRtHist") Then
    wsql3 = " Exists(Select 'x' from RmRtHist where " + IIF(gs_Partition = ctCurrPrtn, " RhPrtKey='" + ctCurrPrtn + "' and ", "") + " RhYyyyMm= '" + CStr(adc("UoYyMmFr")) + "' and " + _
            " RhTcTyp= TsTyp and RhCtg= TsSTyp and RhCmCtg= '" + adc("UoCmCtgFr") + "') " + wCnd + _
            " group by TsTbl, TsTyp, TsSTyp "
  End If
...
Select Case UCase(IdName)
  'Uni.20- Currency code scope can enter only to Tctyp =RM else currency code scope will be invisible
  Case Is = UCase("UOCURCDFR"), UCase("UOCURCDTO"), UCase("UOCURCDSEL")
    If UCase(adc("UOPMCDFR")) = "RM" And UCase(adc("UOPMCDTO")) = "RM" Then
      LblCurrCd.Visible = True
      adc("UOCURCDFR").Visible = True
      adc("UOCURCDTO").Visible = True
      adc("UOCURCDSEL").Visible = True
    Else
      LblCurrCd.Visible = False
      adc("UOCURCDFR").Visible = False: adc("UOCURCDFR") = ""
      adc("UOCURCDTO").Visible = False: adc("UOCURCDTO") = ""
      adc("UOCURCDSEL").Visible = False: adc("UOCURCDSEL") = ""
    End If
End Select
End Sub
