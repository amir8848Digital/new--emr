...
'*** Report Sql ***
  ...
  If UCase(adc.MenuCd) = UCase("RepLclInv") Or UCase(adc.MenuCd) = UCase("RepInvLclRM") Then
  wSqlStrg = "Select InTc, InYy, InChr, InNO, " + ws_IdSr + " as IdSr, Max(InPrnDt) as InDt,  " + _
  ...
  Else
  ' ***** Manali 3.7.1
  wSqlStrg = "Select OmTc as InTc, OmYy as InYY, OmChr as InChr, OmNO as InNO, " + ws_IdSr + " as IdSr, Max(IsNull(InPrnDt, OmDt)) as InDt,  " + _
    ...
    "InHUnResolutions, Max(IsNull(InLclInvTyp, PDesc)) as InLclInvTyp, Max(IsNull(InHCstNo, HCstNo)) as InHCstNo, Max(IsNull(InHVatNo, HVatNo)) as InHVatNo, " + _
  ...
  Else
    ...
    wSqlStrg = wSqlStrg + " From OrdRm " + _
      ...
      " Join Param On PTyp='TC' and PMCd=OdTc " + _
      ...
      " where /*(Select PValue from Param Where PTyp='TC' and PMCd=OdTc)*/ PValue=" + IIF(UCase(adc.MenuCd) = UCase("RepJtLclInv"), "'JT'", "'FG'") + " " + IIF(gs_Partition = ctCurrPrtn, " and OmPrtKey='" + ctCurrPrtn + "' ", "") + IIF(wCnd <> "", " and ", "") + wCnd + " " + _
