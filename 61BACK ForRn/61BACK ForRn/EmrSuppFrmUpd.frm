...
Begin VB.Form EmrSuppFrmUpd 
   ...
   Begin VB.Frame FraMain 
      ...
      Top             =   -150
      Width           =   12090
      Begin VB.CommandButton cmdUpdDmStkNum 
         Caption         =   "Update StkNum"
         CausesValidation=   0   'False
         Height          =   600
         Left            =   7230
         Style           =   1  'Graphical
         TabIndex        =   272
         ToolTipText     =   "Update StkNum"
         Top             =   1380
         Width           =   2400
      End
      ...
      Begin VB.Frame FraEnaDisaMod 
         ...
         Begin MwfCtl.MWCTL_MED atxt 
            ...
            Left            =   3090
            TabIndex        =   221
            ToolTipText     =   "Enable RfId Module in Emr (Yes/No)"
            Top             =   3930
            Width           =   285
            _ExtentX        =   503
            _ExtentY        =   503
...
Private Sub cmdUpdDmStkNum_Click()
  Dim wrsSAStkNum As MDORowSet
  Set wrsSAStkNum = moCn.OpenRes("Select * from OrdDsg where OdTc='JPR' and OdDmStkChr='XY' order by OdCoCd, OdTc, OdYy, OdChr, OdNo, OdSr ")
  Dim wodsadt As Date
  Dim wodsaqty As Long
  Dim woddmcd As String
  Dim wOdDmSz As String
  Dim wOdSfx As String
  Dim woddmstkcocd As String
  Dim woddmstkyy As String
  Dim woddmstkchr As String
  Dim woddmstkno As Long
  Dim wOdPRIdNo As Long
  
  Dim wrsPRStkNum As MDORowSet
  moCn.Execute ("Disable Trigger OrdDsg_InUpDl on OrdDsg")
  Dim wrsPRBalOdQty As Long
  Set wrsPRStkNum = moCn.OpenRes("Select * from OrdDsg where OdTc='JPR' and OdDmStkChr='XY' order by OdCoCd, OdTc, OdChr, OdNo, OdSr asc")
   
'  wrsPRStkNum.MoveFirst
  Do While Not (wrsPRStkNum.EOF Or wrsPRStkNum.BOF)
   
      wrsPRBalOdQty = wrsPRStkNum!OdOrdQty
'      Dim wOdSaDt As Date
      Set wrsSAStkNum = moCn.OpenRes("Select * from OrdDsg where OdTc='JSA' and OdDmStkNo=0 and OdDmCd='" + wrsPRStkNum!OdDmCd + "' and OdDmSz='" + wrsPRStkNum!OdDmSz + "' and OdSfx='" + wrsPRStkNum!OdSfx + "' and OdOmDt>= '" + CStr(wrsPRStkNum!OdOmDt) + "' order by OdCoCd, OdTc, OdChr, OdNo, OdSr asc")
      Do While Not (wrsSAStkNum.EOF Or wrsSAStkNum.BOF)
      
'      If wrsPRStkNum.RecCount > 0 Then
          If wrsPRBalOdQty >= wrsSAStkNum!OdOrdQty Then
            moCn.Execute ("Update OrdDsg set OdDmStkCoCd='" + wrsPRStkNum!OdDmStkCoCd + "', OdDmStkYy='" + wrsPRStkNum!OdDmStkYy + "', OdDmStkChr='" + wrsPRStkNum!OdDmStkChr + "', OdDmStkNo=" + CStr(wrsPRStkNum!OdDmStkNo) + " where OdIdNo=" + CStr(wrsSAStkNum!OdIdNo))
'        moCn.Execute ("Update OrdDsg set OdExpQty= OdExpQty+" + (wrsSAStkNum!odordqty) + " where OdIdNo=" + CStr(wrsPRStkNum!OdIdNo))
 '     End If
            wrsPRBalOdQty = wrsPRBalOdQty - wrsSAStkNum!OdOrdQty
          End If
        wrsSAStkNum.MoveNext
        If wrsPRBalOdQty <= 0 Then Exit Do
      Loop
      wrsPRStkNum.MoveNext
  Loop
  moCn.Execute ("Enable Trigger OrdDsg_InUpDl on OrdDsg")
  MsgBox "DONE"
End Sub
Private Sub OldcmdUpdDmStkNum_Click()
  Dim wrsSAStkNum As MDORowSet
  Set wrsSAStkNum = moCn.OpenRes("Select * from OrdDsg where OdTc='JSA' order by OdCoCd, OdTc, OdYy, OdChr, OdNo, OdSr ")
  Dim wodsadt As Date
  Dim wodsaqty As Long
  Dim woddmcd As String
  Dim wOdDmSz As String
  ...
  wrsSAStkNum.MoveFirst
  Do While Not (wrsSAStkNum.EOF Or wrsSAStkNum.BOF)
      Dim wJPROdIdNo As Long
      moCn.Execute ("Disable Trigger OrdDsg_InUpDl on OrdDsg")
     ' Dim wodsaqty As Long
      wodsaqty = wrsSAStkNum!OdOrdQty
      wodsadt = wrsSAStkNum!OdOmDt
      
'      Dim wOdSaDt As Date
      Set wrsPRStkNum = moCn.OpenRes("Select top 1 OdDmStkCoCd, OdDmStkYy, OdDmStkChr, OdDmStkNo from OrdDsg where OdTc='JPR' and OdDmCd='" + wrsSAStkNum!OdDmCd + "' and OdDmSz='" + wrsSAStkNum!OdDmSz + "' and OdSfx='" + wrsSAStkNum!OdSfx + "' and OdOrdQty - OdExpQty > " + CStr(wrsSAStkNum!OdOrdQty) + " and OdOmDt< '" + CStr(wrsSAStkNum!OdOmDt) + "' order by OdOmDt asc")
      If wrsPRStkNum.RecCount > 0 Then
'        moCn.Execute ("Update OrdDsg set OdDmStkCoCd='" + wrsPRStkNum!OdDmStkCoCd + "', OdDmStkYy='" + wrsPRStkNum!OdDmStkYy + "', OdDmStkChr='" + wrsPRStkNum!OdDmStkChr + "', OdDmStkNo=" + CStr(wrsPRStkNum!OdDmStkNo) + " where OdIdNo=" + CStr(wrsSAStkNumwOdPRIdNo))
        moCn.Execute ("Update OrdDsg set OdExpQty= OdExpQty+" + (wrsSAStkNum!OdOrdQty) + " where OdIdNo=" + CStr(wrsPRStkNum!OdIdNo))
      End If
      wrsPRStkNum.MoveNext
  Loop
...
Private Sub UpdDefDsgAnaCodes()
  ...
  Do While ws_AnaFlds <= GrdDsgAnlyFlds.Rows - 1
    moCn.Execute "Insert Into DsgAna (DaTcTyp,DaCd,DaSz,DaAnaSr,DaAnaCd,DaAnaDesc,ModUsr,ModDt,ModTime,DaDmIdNo,DaPrtKey) " + _
                 " Select DmTcTyp, DmCd , DmSz , '" + GrdDsgAnlyFlds.Value(ws_AnaFlds, "WDANASR") + "', '" + GrdDsgAnlyFlds.Value(ws_AnaFlds, "WDANADEFVAL") + "', '', 'MIC', '" + Format(moCn.SrvrDate, "DD/MM/YY") + "', 0.0,DmIdNo,'C' " + _
                 " From DsgMst " + _
...
Private Function SaveFile() As Boolean
  ...
  For i = LBound(mt.tCoDet) To UBound(mt.tCoDet)
    ...
    ws_Tmp1 = Format$(mt.td_EndDt, "dd/mm/yy") & Trim$(mt.tCoDet(i).ts_CoNm)
...
Private Sub InvFgRmMisMatch()
  ...
  ws_StartTime = "Start Time: " + CStr(Format(moCn.SrvrDate, "DD MMM YYYY")) + " / " + Format(moCn.SrvrTime, "#0.#0")
  DispMsg "Start Time: " + CStr(Format(moCn.SrvrDate, "DD MMM YYYY")) + " / " + Format(moCn.SrvrTime, "#0.#0"), etInfo
  ...
  TxtDisp.Text = ws_StartTime + Chr(13) + " " + TxtDisp.Text + Chr(13) + _
                ...
                " End Time: " + CStr(Format(moCn.SrvrDate, "DD MMM YYYY")) + " / " + Format(moCn.SrvrTime, "#0.#0")
...
",hCell,HCERegnNo,HCHCDHCOnCustRm,hClstrCd,HCstRtEdtYn,HDefRepBagLoc,hDmPrcsSeq,HInAmt10Per,HInAmt11Per,HInAmt8Per,HInAmt9Per,hLocSeq,HRmStkBy,MqrLn3,hStkRtNoDsp,hStkRtNoEdit,hOLORYN,hAddBrkMsgYN,HOrLotYN,hInvByBagYN,hOrdMulByYN,hChkFgWghtYN,hAddJROYN,hChkJEmrMasterRecordYN,HOrdMstShowPicAlwaysYN,hRecalcOrdWght,HcHkoRDxkTyn, hBmOrdHldSeoYN)"
...
",HPdModYN,HPoL,hPrdPtsOnBQtyYN,hPrtKey,HPtrMinusL,HPtrPlusL,HRfIdMod,HRndOffOdTolWtYn,hRtfrRtChrtYN,HRtMinusL,HRtPlusL,hRwrkModYn,HSbUom,HSelfDeductYN,HSelfDscYN,hSetTypOnlyFrOrdYN,hShowBgHistDetsYN " + _
",HShowRepLogo , HTinNo, hTolQtyPctDC, hTolWtPctDC, HTotNoOfPack, hTravelExlYN, HTypOfExporter, HUnitCd, HValidDsgSetCd" + _
",hCell,HCERegnNo,HCHCDHCOnCustRm,hClstrCd,HCstRtEdtYn,HDefRepBagLoc,hDmPrcsSeq,HInAmt10Per,HInAmt11Per,HInAmt8Per,HInAmt9Per,hLocSeq,HRmStkBy,MqrLn3,hStkRtNoDsp,hStkRtNoEdit,hOLORYN,hAddBrkMsgYN,HOrLotYN,hInvByBagYN,hOrdMulByYN,hChkFgWghtYN,hAddJROYN,hChkJEmrMasterRecordYN,HOrdMstShowPicAlwaysYN,hRecalcOrdWght,hChkOrdXKtYN,hBmOrdHldSeoYN " + _
" From Head where HCd= '" + ctSelfCmCd + "' and HCoCd= '" + ADC("wSrcCoCd") + "' "
  ...
  SqlStr = SqlStr + " Insert Into CustMst " + _
            ...
            " CmOmSplRem,CmInvAllBags,CmValidCoCd,CmCell,CmPayTermsCd,CmLkUpMrpDisc,CmInvByBagYN,CmOrdMulByYN,CmTcsYN,CmLkupPDCM ,CmAllwNewSOYN,CmAllwNewSODesc " + _
