...
Dim ws_Val As String
  ...
  If gs_Partition = ctCurrPrtn And gs_PurgeDt <> "01/01/80" And _
      ((CDate(adc("UOOPNYYMM")) <= Format(CDate(gs_PurgeDt), "YYYYMM") And adc("UOOPNYYMM") <> 0) Or _
        (CDate(adc("UODTFR")) <= CDate(gs_PurgeDt) And CDate(adc("UoDtFr")) <> MWLib.EmptyDate) Or _
        (CDate(adc("UODTTO")) <= CDate(gs_PurgeDt) And CDate(adc("UoDtTo")) <> MWLib.EmptyDate)) Then
      DispMsg "From/To Date Or OpenYYYYMM Cannot Be Less Than Purge Date [" + CStr(gs_PurgeDt) + "]", etError: Exit Sub
  End If
  '****** Sachin 3.02.0 - Check on YYYYMM Entered by user ******
  ...
  If gs_Partition = ctCurrPrtn And gs_PurgeDt <> "01/01/80" And CDate(adc("UoDtFr")) = MWLib.EmptyDate Then
    wDtFr = gs_PurgeDt
  Else
    wDtFr = adc("UoDtFr")
  End If
  
  If gs_Partition = ctCurrPrtn And gs_PurgeDt <> "01/01/80" And CDate(adc("UoDtTo")) = MWLib.EmptyDate Then
    wDtTo = gs_PurgeDt
  Else
    wDtTo = adc("UoDtTo")
  End If
  ...
  If adc("UOYN2") = "Y" Then
    ...
    If CInt(Format(wDtFr, "mm")) > 0 And CInt(Format(wDtFr, "mm")) < 12 Then
        wTmpDt = Format$(DateAdd("d", -1, CDate("01/" + Format$(CInt(Format(wDtFr, "mm")) + 1, "00") + "/" + CStr(CInt(Format(adc("UoDtFr"), "yy"))))), "dd/MM/yy")
    ElseIf CInt(Format(wDtFr, "mm")) = 12 Then
        wTmpDt = Format$(DateAdd("d", -1, CDate("01/01/" + CStr(CInt(Format(wDtFr, "yy")) + 1))), "dd/MM/yy")
    End If
    
  End If
  
  If adc("UOYN3") = "Y" Then
    Rep.TxtHead.SetText ("Loose RM Stock Ledger (By Quantity)")
    'Rep.RecordSelectionFormula = "{rdo.qDrQty} <> 0 Or {rdo.qCrQty} <> 0 Or {rdo.qOpnQty} <> 0"
  ElseIf adc("UOYN3") = "N" Then
    Rep.TxtHead.SetText ("Loose RM Stock Ledger (By Weight)")
...
' **** Manali 3.03 - 09/07/08 - 'RisRmSzDesc as qRmSzDesc' addded
  ...
  ' ****** Manali 3.4.1 - 02/12/08 - [UoDtFr, UoDtTo respectively replaced by wDtFr, wDtTo]
  wFrDt = "01/" + Format(wDtFr, "mm") + "/" + Format(wDtFr, "yy")
...
'                  CInt(Format(ADC("UoDtFr"), "yyyy"))
  ...
  wSqlOpnTdFr = "Insert Into #TmpLRmStkLed Select " + grpflds1 + "," + _
                ...
                "OdJLVchRt as qStkRt, (Select PValue3 from Param where PTyp= 'RMSCTG' and PMCd= RmCtg and PSCd= RmSCtg) as qPtrYN, " + _
                "(Case when RmCtg in ('G', 'P', 'S', 'L', 'M') then " + wPureWtOrZWt + " when RmCtg= 'X' then " + wXPureWtOrZwt + " else 0 end)" + wPurityWt + " as qRmPurityWt, " + _
                ...
                "Left Outer Join RmIdSz on RisRmCtg=RmCtg and RisRmId=RmId and RisSz1=OdJLLn1 " + _
                 wDtCndOpnTd + wCnd + IIF(gs_Partition = ctCurrPrtn, " and OmPrtKey='" + ctCurrPrtn + "' ", "") + " And LocTyp= '" + adc("UOLOCTYPFR") + "'" + IIF(wAddlCnd <> "", " And " + wAddlCnd, "") + " Option (MaxDop 1)"
  ...
  wSqlOpnTdTo = "Insert Into #TmpLRmStkLed Select " + grpflds1 + "," + _
                ...
                "       RrLsCtg= '' and RrLabMCd= '' and RrFrLn<= OdJLLn1 and RrToLn>= OdJLLn1 " + _
                " Left Outer Join RmIdSz on RisRmCtg=RmCtg and RisRmId=RmId and RisSz1=OdJLLn1 " + _
                wDtCndOpnTd + wCnd + IIF(gs_Partition = ctCurrPrtn, " and OdPrtKey='" + ctCurrPrtn + "' ", "") + " And LocTyp= '" + adc("UOLOCTYPFR") + "'" + IIF(wAddlCnd <> "", " And " + wAddlCnd, "") + " Option (MaxDop 1)"
  ...
  wSqlDetFr = " Insert Into #TmpLRmStkLed Select " + grpflds1 + "," + _
              ...
              " Left Outer Join RmIdSz on RisRmCtg=RmCtg and RisRmId=RmId and RisSz1=OdJLLn1 " + _
              wDtCndDet + wCnd + IIF(gs_Partition = ctCurrPrtn, " and OmPrtKey='" + ctCurrPrtn + "' ", "") + " And LocTyp= '" + adc("UOLOCTYPFR") + "'" + IIF(wAddlCnd <> "", " And " + wAddlCnd, "") + " Option (MaxDop 1)"
