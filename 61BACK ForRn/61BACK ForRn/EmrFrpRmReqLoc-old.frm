...
' **** Crm 2.12 - 23-02-06 **** '
  ...
  wSqlStrg = " Select " + grpflds + ", " + _
             ...
             " where BCls= 'N' and BQty>0 and  BRjQty = 0 and " + _
             " (IsNull((Select TdTc from Txnd where BLstYyKey= TdYyKey and  TdPrtKey=BPrtKey and TdBIdNo=BIdno and TdCoCd=BCoCd and  TdByy=Byy and TdBChr=Bchr and TdBNo=BNo and TdSrNo=0),'') <> (Select PValue from Param where PTyp='TCTyp' and PMCd='MLT')) " + _
             " and (case when IsNull((select sum(TdRmWt* (Case when TdFrRmLoc='W' and TdFrRmDc='D' then 1 " + _
             ...
             " TdBNo=BNo " + IIF(gs_Partition = ctCurrPrtn, " and TdPrtKey=BPrtKey", "") + " and TdRmCd=OrRmCd), 0) > 0 then 0 else 1 end) = 1 " + wCnd + IIF(gs_Partition = ctCurrPrtn, " and BPrtKey='" + ctCurrPrtn + "' ", "") + _
