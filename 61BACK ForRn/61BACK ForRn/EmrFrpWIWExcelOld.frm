...
Begin VB.Form EmrFrpWIWExcel 
   ...
   Begin VB.Frame fra_nkeys 
      ...
      Begin TabDlg.SSTab TAB_REP 
         ...
         TabPicture(0)   =   "EmrFrpWIWExcel.frx":0000
         Tab(0).ControlEnabled=   -1  'True
         Tab(0).Control(0)=   "fra_tabrep(0)"
         Tab(0).Control(0).Enabled=   0   'False
         Tab(0).ControlCount=   1
         TabCaption(1)   =   "&Group&Sort"
         TabPicture(1)   =   "EmrFrpWIWExcel.frx":001C
         ...
         Begin VB.Frame fra_tabrep 
            ...
            Begin VB.CheckBox ChkBoxArr 
               ...
               TabIndex        =   62
               Tag             =   "Report to Excel"
               ToolTipText     =   "Check to Report Output to Excel"
               Top             =   6000
               Width           =   1890
            End
   ...
   End
End
Attribute VB_Name = "EmrFrpWIWExcel"
...
Private Sub SetGroupSort()
  ...
  '6.1 Ord date, PRd del date, Exp del date
  GRP_REP.Add "Ord Date", "convert(varchar(8), max(OmDt), 112)", "convert(varchar, max(OmDt), " + CStr(DtFmtSlahYyyyVal()) + ")", "", "", "", "", "", ""
  GRP_REP.Add "Prd Del Date", "convert(varchar(8), max(OdDelDt), 112)", "convert(varchar, max(OdDelDt), " + CStr(DtFmtSlahYyyyVal()) + ")", "", "", "", "", "", ""
  ...
  If UCase(ADC.MenuCd) = UCase("RepWIWExp") Or UCase(ADC.MenuCd) = UCase("RepWIWLongExp") Then _
    GRP_REP.Add "Exp Del Date", "convert(varchar(8), max(OdExpDelDt), 112)", "convert(varchar, max(OdExpDelDt), " + CStr(DtFmtSlahYyyyVal()) + ")", "", "", "", "", "", ""
...
'Call AutomateExcelWiw(Me)

'adc("UOCOCDFR") = Trim(ReadCell(1, 2, 2))
'Exit Sub
'*** (Jen 2.12)
  '*** Jay 3.2.0 [PicNm]
...
' ****** Bhavna emr 5.1.0 Wax set and hand set diamonds available for ..If diamond status Y
  ...
  wSqlStrg = "Set DateFormat " + DtFmtStr() + " " + _
             ...
             " Sum(case when 'Y'='Y' Then IsNull(BQty, 0) when 'Y'='N' Then IsNull(1, 0) Else 0 End) as qFlrQty " + wHsQty + wWsQty + _
             wFldLst + ", Replace(max(OdSalRem),char(13)+ char(10),' ') as OdSalRem, '" + CStr(wDetPos) + "' as qDetPos, 'N' as qDispSalRemYN,  " + _
             " IsNull(max(WC.PMCd), '') as qColNm, IsNull(max(RC.PValue), '') as qColCrCd, '' as qAppColYN, " + _
...
'             " Group By OdTc, OdYy, OdChr, OdNo, OdSr "
  ...
  '6.1-23 it call setexcel to show output in excel
  If ADC("UoYN1") = "Y" Then
    Call SetExcel(wSqlStrg)
    Exit Sub
  End If
...
Private Function FldLst(ByVal mSeq As String, ByVal mCtr As Integer)
  ...
  Do While wi_Ctr <= mCtr
    ...
    Else
      ...
      ws_fldlst = ws_fldlst + ",Sum(Case When IsNull(" + IIF(ADC("UOYN2") = "N", "BLoc", "LocPrntCd") + ", '') in(" + wLocForPrcs + ") and 'Y'='Y' Then " + _
                "IsNull(BQty, 0) When IsNull(" + IIF(ADC("UOYN2") = "N", "BLoc", "LocPrntCd") + ", '') in(" + wLocForPrcs + ") and 'Y'='N' Then " + _
                "IsNull(1, 0) Else 0 End) AS qQty" + CStr(wi_Ctr)
      
      Else
      ws_fldlst = ws_fldlst + ",Sum(Case When IsNull(" + IIF(ADC("UOYN2") = "N", "BLoc", "LocPrntCd") + ", '')='" + wReqStr + "' and 'Y'='Y' Then " + _
                "IsNull(BQty, 0) When IsNull(" + IIF(ADC("UOYN2") = "N", "BLoc", "LocPrntCd") + ", '')='" + wReqStr + "' and 'Y'='N' Then " + _
                "IsNull(1, 0) Else 0 End) AS qQty" + CStr(wi_Ctr)
      End If
...
'  If adc("UoYN7") = "N" Then OptQty(1).Value = True
  ...
  ElseIf ADC("UOLOCTYPFR") = "R" Then
    OptLoc(1).Value = True
  End If
  '6.1-23 default value for report to excel is No
  ADC("UoYN1") = "N"
  If ADC("UoYN1") = "Y" Then ChkBoxArr(0).Value = Checked Else ChkBoxArr(0).Value = Unchecked
...
'mRep.wCoCd.SetText gs_CoNm
  ...
  If ws_MultiCoMod = True Then
    ...
    mRep.TxtCoCdTo.Suppress = False ': mRep.TxtCoCdSel.Suppress = False   '578.99.c CoCd selection removed
    mRep.TxtCoCdToLbl.Suppress = False ': mRep.TxtCoCdSelLbl.Suppress = False   '578.99.c CoCd selection removed
    mRep.TxtCoCdTo.SetText ADC("UOCOCDTO")
    'mRep.TxtCoCdSel.SetText ADC("UOCOCDSEL")     '578.99.c CoCd selection removed
  Else
    ws_HName = moCn.GetFldVal("Select HName from Head where HCoCd ='" + ADC("UoCoCdFr") + "' and " + _
                                                      "HCd = '" + ctSelfCmCd + "' ")
    mRep.wCoCd.SetText ws_HName
    mRep.TxtCoCdFr.SetText ADC("UOCOCDFR")
    mRep.TxtCoCdTo.Suppress = True ' : mRep.TxtCoCdSel.Suppress = True    '578.99.c CoCd selection removed
    mRep.TxtCoCdToLbl.Suppress = True ': mRep.TxtCoCdSelLbl.Suppress = True   '578.99.c CoCd selection removed
  End If
  '***** Sachin 2.14.0 - [24-11-2006] - [Multi-Company] ******
...
On Error GoTo Err_Hndlr:
  ...
  FileCopy "C:\MwApp\Xl_WIW.xlsx", sFilePath
  Set oBook = oExcel.Workbooks.Open(sFilePath, Password:=ctXCelPwd, WriteResPassword:=ctXCelWritePwd)
  Set oSheet = oBook.worksheets(1)
  oExcel.Visible = False: oExcel.DisplayAlerts = False
  
  Dim wRsExcel As MDORowSet, n As Integer
  Set wRsExcel = moCn.OpenRes(pSqlStr)
  If wRsExcel.RecCount = 0 Then Exit Sub
  n = 4
  With wRsExcel
    .MoveFirst
    Do While Not (wRsExcel.EOF Or wRsExcel.BOF)
      oSheet.Cells(n, 1) = wRsExcel!OdCoCd + "/" + wRsExcel!OdTc + "/" + wRsExcel!OdYy + "/" + wRsExcel!OdChr + "/" + CStr(wRsExcel!OdNo)
      oSheet.Cells(n, 2) = wRsExcel!OdSr
      oSheet.Cells(n, 3) = wRsExcel!OdDmCd
      oSheet.Cells(n, 4) = wRsExcel!OdDelDt
      oSheet.Cells(n, 5) = wRsExcel!OmCmCd
      oSheet.Cells(n, 6) = wRsExcel!OdKt
      oSheet.Cells(n, 7) = wRsExcel!OdDmCol
      oSheet.Cells(n, 8) = wRsExcel!OdOrdQty
      oSheet.Cells(n, 9) = wRsExcel!qBalQty
      oSheet.Cells(n, 10) = wRsExcel!qFlrQty
      oSheet.Cells(n, 11) = wRsExcel!qQty1
      oSheet.Cells(n, 12) = wRsExcel!qQty2
      oSheet.Cells(n, 13) = wRsExcel!qQty3
      oSheet.Cells(n, 14) = wRsExcel!qQty4
      oSheet.Cells(n, 15) = wRsExcel!qQty5
      oSheet.Cells(n, 16) = wRsExcel!qQty6
      oSheet.Cells(n, 17) = wRsExcel!qQty7
      oSheet.Cells(n, 18) = wRsExcel!qQty8
      oSheet.Cells(n, 19) = wRsExcel!qQty9
      oSheet.Cells(n, 20) = wRsExcel!qQty10
      oSheet.Cells(n, 21) = wRsExcel!qQty11
...
End Sub
Private Sub SetExcel(ByVal pSqlStr As String)
On Error GoTo Err_Hndlr:
'  Dim sFilePath As String
'  Dim oExcel, oBook, oSheet
'  Dim wRsOrdMst As MDORowSet, wRsDsgDet As MDORowSet
...
'
  ...
  wSrcFile = "C:\MwApp\Xl_WIW.xlsx"
  wFileNm = Format(Now(), "YYYYmmDD") + ".xlsx"
  'wFileNm = Format(Now(), "YYYYmmDD") + "_01.xlsm"
  wTgtFile = "C:\MwApp\Xl_WIW_" + wFileNm
...
'    Loop
  ...
  Dim wRsExcel As MDORowSet, n As Integer
  Set wRsExcel = moCn.OpenRes(pSqlStr)
  If wRsExcel.RecCount = 0 Then DispMsg "No Record Found for Given Scope.", etInfo: CloseExcel: Exit Sub
...
'  FileCopy "C:\MwApp\Xl_WIW.xlsx", sFilePath
  'Set oBook = oExcel.Workbooks.Open(sFilePath, Password:=ctXCelPwd, WriteResPassword:=ctXCelWritePwd)
'  Set oBook = oExcel.Workbooks.Open(Cd1.FileName)
'  Set oSheet = oBook.worksheets(1)
'  oExcel.Visible = False: oExcel.DisplayAlerts = False
'
'  Dim wRsExcel As MDORowSet, n As Integer
