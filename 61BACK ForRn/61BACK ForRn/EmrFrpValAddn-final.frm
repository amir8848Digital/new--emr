...
' **** Zubin 211 **** '
  ...
  wStr1 = "Select IDENTITY(INT, 1, 1) as qTzSrNo, max(TBOENo) as TBOENo, " + _
          ...
          "M.TzRmZCd+ Space(12-Len(M.TzRmZCd))+ Str(IzRmZRt, 9, 3) as qTzZCdRt, " + _
  ...
  wStr2 = wStr2 + "sum((case when IrMainMet= 'Y' then " + wLsWtStr + " else (" + wMetWtStr + "* IdGldLs/100) end)* (case when Rm.RmCtg in ('G') then 1 else 0 end)) as qGldLsWt, " + _
          ...
          "IrRmZCd+ Space(12-Len(IrRmZCd))+ Str(IrRmZPureRt, 9, 3) as qIrZCdRt, IrRmZPureRt as qSalRt, " + _
  ...
  'RMCD changed as ZCODE, RMINTQLY changed as ZINTQLY
  wGrpByStr = IIF(ADC("UoRmGrp") = "ZCODE", "IrRmZCd", IIF(ADC("UoRmGrp") = "ZINTQLY", "(case when RmMst.RmIntQly='' then IrRmZCd else RmMst.RmIntQly end)", "c.RmSCtg"))
...
'      .LineRFExt2.Suppress = True
    ...
    End If
    '*** (Jen 2.13)
    'Diagold.6 In ValAddnDTA report if Rmdetail level selected as ZCODE/ZINTQLY then font size will be 5
    ' otherwise no change in font size
    'RMCD changed as ZCODE, RMINTQLY changed as ZINTQLY
    If UCase(ADC.MenuCd) = UCase("RepValAddnDTA") And (ADC("UoRmGrp") = "ZCODE" Or ADC("UoRmGrp") = "ZINTQLY") Then
      .SubRep2_RmSCtg1.Font.Size = 5
      Rep.hStoneType.SetText (IIF(ADC("UoRmGrp") = "ZCODE", "Code", "Int.Qly"))
