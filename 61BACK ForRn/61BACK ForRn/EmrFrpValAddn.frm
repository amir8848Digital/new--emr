      ...
      Begin TabDlg.SSTab TAB_REP 
         ...
         Begin VB.Frame fra_tabrep 
            ...
            Begin VB.CheckBox ChkBoxArr 
               ...
               TabIndex        =   25
               Tag             =   "ShowXBoe"
               ToolTipText     =   "Highlight VA%age  (Yes / No)"
               Top             =   5660
               Width           =   2220
            End
            Begin VB.CheckBox ChkBoxArr 
            ...
            Begin MwfCtl.MWCTL_MED atxt 
               ...
               Left            =   2040
               TabIndex        =   8
               ToolTipText     =   "RMCD means Customs Code & RMINTQLY means Customs Int.Qly"
               Top             =   2070
               Width           =   1305
               _ExtentX        =   2302
               _ExtentY        =   503
            ...
            Begin MwfCtl.MWCTL_MED atxt 
               ...
               Left            =   2040
               TabIndex        =   26
               ToolTipText     =   "Enter Value to Highlight Va %age Below "
               Top             =   5955
               Width           =   1635
               _ExtentX        =   2884
               _ExtentY        =   503
            ...
            Begin MwfCtl.MWCTL_MED atxt 
               ...
               Enabled         =   0   'False
               DataType        =   4
               IdName          =   "WHIGHLIGHTVAYN"
            End
            Begin VB.Label LblVAPer 
               BackStyle       =   0  'Transparent
               Caption         =   "VA %age Below"
               BeginProperty Font 
                  Name            =   "Arial"
                  Size            =   9.75
                  Charset         =   0
...
'  from InvDsg, OrdDsg, DsgMst, InvRm, OrdRm, RmMst, Txn, Txndz where 1= 2
  ...
  'Hk.10 when Hide Col 13-16 option or Show Profit % option is selected,we cannot select Highlight VA%age
  If (ADC("UoYn2") = "Y" Or ADC("UoYn5") = "Y") And ADC("wHighlightVAYn") = "Y" Then
    DispMsg "Highlight VA%age option should be blank when Hide Col 13-16/ Show Profit % is selected", etError
    Exit Sub
  End If
...
' **** Zubin 211 **** '
  ...
  '578.19 space(12-Len(M.TzRmZCd)) changed to space(16-Len(M.TzRmZCd))
  wStr1 = "Select IDENTITY(INT, 1, 1) as qTzSrNo, max(TBOENo) as TBOENo, " + _
          "max(TBOEDt) as TBOEDt, M.TzRmZCd as TzRmZCd,  max(B.TzRmCdDesc) as qTzRmCdDesc,M.TzPurRt as qCstRt, " + _
          "M.TzRmZCd+ Space(16-Len(M.TzRmZCd))+ Str(IzRmZRt, 9, 3) as qTzZCdRt, " + _
  ...
  If moCn.GetFldVal("Select hExpLocal from Head where HCoCd= '" + gs_CoCd + "' and HCd= '" + ctSelfCmCd + "'") = "Y" Then
    ws_qIrWt = " sum((case when IrMainMet= 'Y' then IdCustmMtchWt else IrRmZWt end) " + _
               ...
               "            * (1 + (case when IrMainMet= 'Y' then 0 else (case when Rm.RmCtg<>'M' then IdGldLs else 0 end) end)/ 100) " + _
  ...
  '578.19 Space(12-Len(IrRmZCd)) changed to Space(16-Len(IrRmZCd))
  wStr2 = wStr2 + "sum((case when IrMainMet= 'Y' then " + wLsWtStr + " else (" + wMetWtStr + "* IdGldLs/100) end)* (case when Rm.RmCtg in ('G') then 1 else 0 end)) as qGldLsWt, " + _
          ...
          "IrRmZCd+ Space(16-Len(IrRmZCd))+ Str(IrRmZPureRt, 9, 3) as qIrZCdRt, IrRmZPureRt as qSalRt, " + _
  ...
  wStr3 = "Select qIrSrNo, IdSr, IdDmCd, IdDmSz, IdQty, IdGldLs, qIrWt, qGrsWt, qNetWt,  " + _
          ...
          "DmSalCtg, DmUom, OdDmCol, OrRmCtg, IdTrayNo, qTzSrNo, TBOENo, TBOEDt, qCstRt, qTzWt ,qTzRmCdDesc " + _
          "into #TMPC " + _
  ...
  ' ***** Manali 3.03 - 30/06/08 - Pd Mod - Pd Fields added
  ' ***** Manali 3.8.0 - IdTrayNo added in Sort, xIrRmQty added (ValAddnDTA)
  Dim wGrpByStr As String
  'diagold.6 qTzRmCdDesc changed as IrRmZCd
  '5713 value for RMINTQLY changed
  wGrpByStr = IIF(ADC("UoRmGrp") = "RMCD", "IrRmZCd", IIF(ADC("UoRmGrp") = "RMINTQLY", _
                "case when CHARINDEX('{',c.qTzRmCdDesc)<>0 and CHARINDEX('}',c.qTzRmCdDesc)<>0 and substring(c.qTzRmCdDesc,CHARINDEX('{',c.qTzRmCdDesc)+1,(CHARINDEX('}',c.qTzRmCdDesc)-CHARINDEX('{',c.qTzRmCdDesc))-1)<>'' then " + _
                  " substring(c.qTzRmCdDesc,CHARINDEX('{',c.qTzRmCdDesc)+1,(CHARINDEX('}',c.qTzRmCdDesc)-CHARINDEX('{',c.qTzRmCdDesc))-1) " + _
                    " else (case when RmMst.RmIntQly='' then IrRmZCd else RmMst.RmIntQly end) end ", "c.RmSCtg"))
  
  wStr4 = "Insert into #TMPD Select qIrSrNo, IdSr, IdDmCd, IdDmSz, IdQty, IdGldLs, qIrWt, qGrsWt, qNetWt, " + _
          "qGldWt, qPlWt, qSlWt, qPdWt, qMWt, qNetPureWt, qGldNetWt, qPlNetWt, qSlNetWt, qPdNetWt, qMNetWt, qGldLsWt, qPlLsWt, qSlLsWt, qPdLsWt, qMLsWt, qXWt, qDCSWt, qXIrRmQty, c.RmCtg, " + _
          IIF(UCase(ADC.MenuCd) = UCase("RepValAddnDTA"), wGrpByStr, "c.RmSCtg") + _
          ",IrRmZCd, qTyp, " + _
  ...
  wStr5 = "qTzWt- (case when IsNull((Select sum(qIrWt) from #TMPB b where b.qIrSrNo< c.qIrSrNo " + _
          ...
          "                  IsNull((Select sum(qTzWt) from #TMPA a where a.qTzSrNo< c.qTzSrNo " + _
          "                          and a.qTzZCdRt= c.qIrZCdRt), 0) " + _
          "             else 0 end) as qFinalAvl from #TMPC c join RmMst on RmCd = IrRmZCd "
  ...
  wSqlStrg2 = "Select qIrSrNo, IdSr, IdDmCd, IdDmSz, IdQty, IdGldLs, qIrWt, qGrsWt, qNetWt, " + _
              ...
              "'' as qDmCtgDesc, '' as qDmSalCtgDesc, '' as qDmColDesc, '' as qDmKtDesc, '' as qVaCtgDesc, " + _
  ...
  ws_SubRep = "Select IsNull(PSCd, 'ZClrStn') as qStnCls, " + _
              ...
              "where RmCtg in ('D','C') and RmZ= 'Y' " + _
              "Group By PMCd, PSCd Order By PMCd desc, qStnCls "
...
' ******* Manali - This part is shifted to DispCoNm
  ...
  If UCase(ADC.MenuCd) = UCase("RepValAddnDta") Or UCase(ADC.MenuCd) = UCase("RepStudStmt") Then
    ...
    'diagold.6 if qty>99999 then font size will be 7 else  no change in font size
    'it is common for all Rmdetail level
    Dim ws_QtyChk As String
    ws_QtyChk = "Select qQty from (" + ws_StudDet + ") TmpQtyChk where qQty >99999"
    If moCn.RecSeek(ws_QtyChk) Then Rep.SubRep2_qQty2.Font.Size = 7
...
'      .LineRFExt2.Suppress = True
    ...
    End If
    '*** (Jen 2.13)
    'Diagold.6 In ValAddnDTA report if Rmdetail level selected as RmCd/RmIntQly then
    'font size will be 5, height-170, top-17  otherwise no change
    If UCase(ADC.MenuCd) = UCase("RepValAddnDTA") And (ADC("UoRmGrp") = "RMCD" Or ADC("UoRmGrp") = "RMINTQLY") Then
      .SubRep2_RmSCtg1.Font.Size = 5
      .SubRep2_RmSCtg1.Height = 170
      .SubRep2_RmSCtg1.Top = 17
      Rep.hStoneType.SetText (IIF(ADC("UoRmGrp") = "RMCD", "Code", "Int.Qly"))
      '578.19 calling function to change Rmcode font size
      SetSubFormula Rep.SubRep2, "wRmCdFontSz", GetRmcdFontSz("{rdo.RmSCtg}", 5)
      
    End If
        If UCase(ADC.MenuCd) <> UCase("RepValAddn") Then
      .G2HVABoxBottom.Suppress = True
      .G2HVABoxTop.Suppress = True
    End If
    
    If UCase(ADC.MenuCd) = UCase("RepValAddn") Then
      'Hk.10 if highlight VA % is not selected then Box section for highlight VA% is suppressed
      If ADC("wHighlightVAYn") <> "Y" Then
        .G2HVABoxBottom.Suppress = True
        .G2HVABoxTop.Suppress = True
      End If
      'Hk.10 passing highlight VA % value to DSr
      GRP_REP.SetFormula Rep, "wVAVAl", ADC("wVAPer")
    End If
...
Private Sub ApportnTbl()
  ...
  '578.19 IrRmZCd     varchar(12) changed to IrRmZCd     varchar(16),
  'qIrZCdRt    varchar(21) changed to qIrZCdRt    varchar(25)
  wStr2 = "   qGldLsWt    float           null, " + _
          ...
          "   RmSCtg      varchar(20)      null, " + _
          "   IrRmZCd     varchar(16)     null, " + _
          "   qTyp        varchar(50)     null, " + _
          "   IrRmQty     integer         null, " + _
          "   qSalRt      float           null, " + _
          "   qIrZCdRt    varchar(25)     null, " + _
...
With ChkBoxArr(Index)
  ...
  Case Is = 8     'Hk.10  Tag = Highlight VA %age
    If .Value = Checked Then ADC("wHighlightVAYn") = "Y" Else ADC("wHighlightVAYn") = "N"
...
''''    End If
    ...
    If UCase(ADC.MenuCd) <> UCase("RepValAddn") Then
      ...
      ChkBoxArr(5).Enabled = False: ChkBoxArr(5).Visible = False
        ...
        End If
      'Hk.10 Highlight VA % scopes visible only for ValueAddn report
      ChkBoxArr(8).Enabled = False: ChkBoxArr(8).Visible = False
      ADC("wVAPer").Visible = False: LblVAPer.Visible = False
      
    End If
    ' ***** Manali 3.8.0
    'Diagold.6 - RmDetail Level scope visible only for DTA report
    If UCase(ADC.MenuCd) <> UCase("RepValAddnDTA") Then
...
Private Sub atxt_FldChange(Index As Integer, ByVal IdName As String, Cancel As Boolean, ErrMsg As String, ByVal pv_NewValue As Variant)
 ...
 'urmi 2.04-3
  ...
  'Hk.10 when highlight VA%age is not selected then VA% value should be 0
  Case Is = UCase("wHighlightVAYn")
    If pv_NewValue = "N" Then ADC("wVAPer") = 0
  End Select
...
Private Sub ATXT_FldWhen(Index As Integer, ByVal IdName As String, Cancel As Boolean, ErrMsg As String)
 ...
 'urmi 2.04-3
  ...
  'Diagold.6 calling help of DTA options
  Case Is = UCase("UoRmGrp")
    gs_RmGrp = "DTA"
  'Hk.10 when highlight VA%age is not selected, can't enter VA% value
  Case Is = UCase("wVAPer")
    If ADC("wHighlightVAYn") = "N" Then Cancel = True: ErrMsg = "Cannot Enter value when Highlight VA%age is blank"
  End Select
...
' **** Zubin 211 **** '
  ...
  'Hk.10 default values for highlight VA % scope
  If ADC("wHighlightVAYn") = "" Then ADC("wHighlightVAYn") = "N"
  ADC("wVAPer") = 0
  ...
  ' ****** Manali 3.5.0 - 21/11/08 - Check Boxes added
  'Hk.10 Highlight VA % check as per text box value
  If ADC("wHighlightVAYn") = "Y" Then ChkBoxArr(8).Value = Checked Else ChkBoxArr(8).Value = Unchecked
