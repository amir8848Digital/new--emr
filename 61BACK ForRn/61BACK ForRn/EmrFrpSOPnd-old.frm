      ...
      Begin TabDlg.SSTab TAB_REP 
         ...
         Begin VB.Frame fra_tabrep 
            ...
            Begin MwfCtl.MWCTL_MED atxt 
               ...
               Left            =   1725
               TabIndex        =   41
               ToolTipText     =   "Enter From Design Code Id"
               Top             =   4410
               Width           =   885
               _ExtentX        =   1561
               _ExtentY        =   503
            ...
            Begin MwfCtl.MWCTL_MED atxt 
               ...
               Left            =   4410
               TabIndex        =   42
               ToolTipText     =   "Enter To Design Code Id"
               Top             =   4410
               Width           =   885
               _ExtentX        =   1561
               _ExtentY        =   503
...
If UCase(adc.MenuCd) <> UCase("RepSOPndRM") Then
  ...
  If UCase(adc.MenuCd) <> UCase("RepJtPndRm") And UCase(adc.MenuCd) <> UCase("RepSOPndRM") Then
  ...
  If UCase(adc.MenuCd) = UCase("RepSoPnd") Then
    ...
    GRP_REP.Add "Sales Exec", "CmSalPer", "", "", "", "", "", "", "(Select PDesc from Param Where PTyp = 'SP' and PMCd= CmSalPer) "
  ...
  If UCase(adc.MenuCd) = UCase("RepSOPndRM") Then
      ...
      GRP_REP.Add "Po No", "OdCoCd+max(OmPoNo)", "'['+OdCoCd+'] '+max(OmPoNo)", "OmPoNo", "hInExpNo", "Company Code,Inv Date,Inv Vch No,Invoice Cust,Customer Grp,Customer Typ", "", "", ""
      GRP_REP.Add "Order Date", "convert(varchar(10), OmDt, 112)", "convert(varchar(8), OmDt, 3)", "OmDt", "hOmDt", "Quarter,Month,Week", "", "", ""
      GRP_REP.Add "Design Code", "max(B.OdDmCd)", "", "OdDmCd", "hIdDmCd", "Design Ctg,Sales Ctg", "", "", "max(DmDesc) "
      GRP_REP.Add "Design Size", "max(OdDmSz)", "", "OdDmSz", "hIdDmSz", "", "", "", "(Select PDesc From Param Where PTyp= 'DMSZ' and PmCd= max(OdDmSz))"
      GRP_REP.Add "Suffix", "max(OdSfx)", "", "OdSfx", "hIdSfx", "", "", "", ""
      GRP_REP.Add "Invoice Cust", "max(OmCmCd)", "", "", "", "Customer Grp,Customer Typ", "", "", "(Select (case when (Select PNum from Param where Ptyp = 'USR' and PmCd = '" + Trim(EmrFrmMDI.TXT_UCD.text) + "')  <= 5 then  CmName else  max(OmCmCd) end) From CustMst Where CmCtg= 'C' and CmCd= max(OmCmCd)) "
      ...
      GRP_REP.Add "Karat", "max(OdKt)", "", "OdKt", "hIdKt", "", "", "", "(Select PDesc From Param Where PTyp= 'KT' and PMCd = max(OdKt)) "
      ...
      GRP_REP.Add "Sales Exec", "max(C.CmSalPer)", "", "", "", "", "", "", "(Select PDesc from Param Where PTyp = 'SP' and PMCd= max(C.CmSalPer)) "
...
' ###########################################  Manoj  ###########################################
 If UCase(adc.MenuCd) = UCase("RepSOPndRM") Then adc("UoAsOfDt") = "01/01/80"
...
'  InExpNo, InCmCd, IdQty, idiFOB
  ...
  If UCase(adc.MenuCd) <> UCase("RepJtPndRm") Or UCase(adc.MenuCd) <> UCase("RepSOPndRM") Then
    wSqlStrg = "Set Dateformat DMY Select " + grpflds + ", " + _
             ...
             "B.OdDmSz as OdDmSz, B.OdDmStkYy+'/'+B.OdDmStkChr+'/'+ltrim(str(B.OdDmStkNo)) as OdDmStkNum, B.OdPicNm as OdPicNm, /*B.OdOrdQty*/ " + wOrdQty + " as OdOrdQty, " + IIF(wb_InBaseCur, " (Case when BOm.OmMulDiv= 'M' then (B.OdSalPrc/BOm.OmCnvFct) else (B.OdSalPrc*BOm.OmCnvFct) end) ", "B.OdSalPrc") + " as OdSalPrc, " + _
  ...
  ElseIf UCase(adc.MenuCd) <> UCase("RepSOPndRM") Then
    wSqlStrg = "Select " + grpflds + ", " + _
            ...
            " B.OdDmCd as OdDmCd, B.OdSfx as OdSfx, B.OdDmSz as OdDmSz, B.OdDmStkYy+'/'+B.OdDmStkChr+'/'+ltrim(str(B.OdDmStkNo)) as OdDmStkNum, B.OdPoNo as OdPoNo, DmCtg, OdOrdQty, OdSalPrc, " + wBalQty + " as qBalQty, " + _
...
'             IIF(wAddlCnd <> "", IIF(wCnd <> "", " And ", " Where ") + wAddlCnd, "") + wCrmCnd
  ...
  If UCase(adc.MenuCd) = UCase("RepSOPndRM") Then
     ...
     End If
      ...
      wStr1 = "Select " + grpflds + ", " + _
          ...
          "max(OmCmCd) as InCmCd, 0 as IdExpOdTc, " + _
          ...
          "max(OdVaCtg) as IdVaCtg, max(OdOrdQty) as IdQty, max(" + wIdFOBOpt + ") as IdIFob, " + _
          ...
          " and  OLYy= OdYy and OLChr= OdChr and OLNo= OdNo and OLSr= OdSr), 0) as qLabVal, " + _
          "sum(case when IsNull(RmCtg, '') in ('D', 'C') then IsNull(OrWt, 0)/ 5 else IsNull( OrWt, 0) end) as qGrWt, " + _
          "sum(case when IsNull(RmCtg, '') in ('G', 'P', 'S', 'L') then IsNull(OrWt, 0) else 0 end) as qMetWt, " + _
          "sum(case when IsNull(RmCtg, '') in ('D') then IsNull(OrWt, 0) else 0 end) as qDWt, " + _
          "sum(case when IsNull(RmCtg, '') in ('C') then IsNull(OrWt , 0) else 0 end) as qCWt, " + _
          "sum(case when IsNull(RmCtg, '') in ('X') then IsNull(OrWt, 0) else 0 end) as qXWt " + _
  ...
  wStr2 = "From OrdDsg B join OrdMst BOm on OmIdNo=OdOmIdNo and OmCoCd= OdCoCd and OmTc= OdTc and OmYy= OdYy " + _
          ...
          " OrChr= OdChr and OrNo= OdNo and OrSr= OdSr " + IIF(gs_Partition = ctCurrPrtn, " and OrPrtKey=OdPrtKey ", "") + " left outer join RmMst on RmCd= OrRmCd " + IIF(gs_Partition = ctCurrPrtn, " and RmPrtKey=OrPrtKey  ", "") + _
           wCnd + wAllPndCnd + wBSCnd + IIF(gs_Partition = ctCurrPrtn, " and OdPrtKey='" + ctCurrPrtn + "' ", "") + " group by OdPrtKey, OdCoCd, OdTc, OdYy, OdChr, OdNo, OdSr Order by OdCoCd, OdTc, OdYy, OdChr, OdNo, OdSr"
...
'    Rep2.LblAsOfDt.Suppress = True
  ...
  ElseIf UCase(adc.MenuCd) = UCase("RepJtPnd") Or UCase(adc.MenuCd) = UCase("RepJtOpnOrd") Then
    If UCase(adc.MenuCd) = UCase("RepJtPnd") Then
      ...
      Rep.TxtHead.SetText "Pending " + wTcDesc ' "Trading Voucher Pending (" + adc("UoOmTc") + ")"
    ElseIf UCase(adc.MenuCd) = UCase("RepJtOpnOrd") Then
      Rep.TxtHead.SetText "Open Order (w.r.t. JPO)"
    End If
  ...
  ElseIf UCase(adc.MenuCd) = UCase("RepJtPndRm") Then
    wTcDesc = moCn.GetFldVal("Select PDesc from Param Where PTyp='TC' and PMCd='" + adc("UoOmTc") + "' and PValue='JT' ")
    Rep1.TxtHead.SetText "Pending " + wTcDesc + " (Raw Material)"
...
Private Sub atxt_ReCalculate(Index As Integer, ByVal IdName As String, ByVal OldVal As Variant, ByVal RowNum As Integer, ByVal SourceName As String, ByVal SourceColName As String)
  ...
  Case Is = UCase("UoOmTcFr")
    If UCase(adc.MenuCd) <> UCase("RepSoPnd") And UCase(adc.MenuCd) <> UCase("RepSOPndRM") Then
...
Private Sub GRP_REP_LostFocus()
  If UCase(adc.MenuCd) = UCase("RepSoPnd") Or UCase(adc.MenuCd) = UCase("RepSOPndRM") Then Call moGrph.SetDefaVal
