...
> `AGENTS.md` in this directory should be a **symlink** to this file — one source of truth, and Codex
...
> auto-loaded. Recreate it with `ln -s AGENT.md AGENTS.md` (verified missing on a Linux checkout,
> 2026-09-09).
...
| Path | What it holds |
|---|---|
| `emr_node/AGENTS.md` | Doctype folder anatomy, framework entry points, backend conventions |
| `Basecamp_Frontend/AGENTS.md` | Render pipeline, `RHFFormFactory` field categories, modal architecture |
...
| `emr_node/.Codex/commands/` | `/migrate`, `/migrate-v3`, `/parse-vb6`, `/verify-form`, `/generate-doc`, … (user-typed only) |
| `emr_node/.Codex/skills/` | `vb6-formconfig-generation` (**.frm + screenshot → formConfig/meta**), `vb6-parity-checklist`, `vb6-ui-parity`, `doctype-child-tables`, `doctype-table-extension`, `doctype-button-types`, `doctype-record-lock`, `doctype-grid-addrow-autofocus`, `addon-menus-reports` |
...
| `Basecamp_Frontend/.Codex/skills/` | `rhf-form-button-types` |

Skills and each repo's `AGENTS.md` are directory-scoped and load automatically when working under
...
`emr_node/.Codex/skills/sql-dialect-portability/SKILL.md`. Trigger-level rules are a separate
concern — `emr_node/.Codex/skills/db-trigger-migration/SKILL.md`.
...
`.Codex/launch.json` at this level defines **`basecamp-dev`** — the Next.js frontend on port 3001.
Start it with the preview tooling, never with a raw `npm run dev` in Bash. The backend has its own
launch config under `emr_node/.Codex/`.
